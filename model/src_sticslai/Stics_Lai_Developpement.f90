...
! subroutine laidev(sc,p,pg,itk,t,soil,c,sta)
 ...
 subroutine laidev(n,                               &  ! IN
  ...
  real,    intent(INOUT)                        :: masec      !> // above ground drymatter // t ha-1
...
! 0.4 local Variables
 ...
 !     endif
    ...
    !    So here , we can only use the data at the date of nsencour + 1 
         ...
         if (P_culturean /= 1 .AND. nsencour /= 0 .AND. nsencour > year_length_in_days) then 
   
            if (n == nsencour - year_length_in_days + 1) then
               nsendltams = dltams
               nsendltai  = deltai
            ...
            if (n == nsencour- year_length_in_days + 2) then
               nsenndurvie = durvie
            endif
         endif
