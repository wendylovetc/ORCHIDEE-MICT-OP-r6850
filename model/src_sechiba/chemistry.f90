...
MODULE chemistry
  ...
  USE pft_parameters
  USE grid
  USE time, ONLY : one_day, dt_sechiba, julian_diff, one_hour, one_year, year_length_in_days
  USE ioipsl_para 
  USE xios_orchidee
    ...
    IF ( ok_radcanopy ) THEN
       ...
       DO ji = 1, kjpindex
          IF (coszang(ji) .GT. zero) THEN
             ...
             So(ji) = Sct*( un + 0.033*COS(360.*pi/180.*julian_diff/year_length_in_days))*coszang(ji)
       ...
       DO ji = 1, kjpindex
          ...
          IF (ok_cropsfertil_NOx) THEN
             ...
             IF (((lalo(ji,1) .GT. 30.) .AND. (julian_diff .GE. 121. .AND. julian_diff .LE. 243.).AND. &
                ...
                ! 1/(year_length_in_days/12*24*60*60*4) for conversion from year to seconds corrected for 4 months of emissions
                flx_fertil_no(ji,jv) = (N_qt_WRICE_pft(ji,jv)*(1/30.)+N_qt_OTHER_pft(ji,jv))*(2.5/100.) &
                     & *1e12/(year_length_in_days*24*60*60*4/12)/(area2(ji)*veget_max(ji,jv))
                ! OR Tropical regions all the year
             ELSE IF ((lalo(ji,1) .GE. -30.).AND.(lalo(ji,1) .LE. 30.).AND.(veget_max(ji,jv) .GT. min_sechiba)) THEN
                flx_fertil_no(ji,jv) = (N_qt_WRICE_pft(ji,jv)*(1/30.)+N_qt_OTHER_pft(ji,jv))*(2.5/100.) &
                     & *1e12/(year_length_in_days*24*60*60)/(area2(ji)*veget_max(ji,jv))
             END IF
