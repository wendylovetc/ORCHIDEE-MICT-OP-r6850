...
!     & rsol, evap_bare_lim, evap_bare_lim_pft, evapot, evapot_corr, snow, flood_frac, flood_res, frac_nobio, snow_nobio, totfrac_nobio, &
     & swnet, swdown, coszang, ccanopy, humrel, veget, veget_max, lai, laitop, laidow, VPD, &!!xchen
...
!    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)  :: evap_bare_lim_pft    !! Limit to the bare soil evaporation when the 
    ...
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out) :: laitop           !!xchen sunlit Leaf area index @tex ($m^2 m^{-2}$) @endtex
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out) :: laidow            !!xchen shade Leaf area index @tex ($m^2 m^{-2}$) @endtex
...
!! xchen add vpd for gpp simulation in Amazon
    REAL(r_std), DIMENSION(kjpindex),     INTENT (out)       :: VPD              !! Vapor Pressure Deficit (kPa)
    ...
    !!REAL(r_std),DIMENSION (kjpindex)         :: VPD                             
    REAL(r_std),DIMENSION (kjpindex)         :: SVP
    REAL(r_std), DIMENSION(kjpindex)      :: air_relhum                          !! air relative humidity at 2m 
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_gsmean           !! mean stomatal conductan 
  ...
  !! 5. Calculate the beta coefficient for transpiration
    ...
    IF ( ok_co2 ) THEN
      ...
      CALL diffuco_trans_co2 (kjpindex, lalo, swdown, pb, qsurf, q2m, t2m, temp_growth, rau, u, v, q_cdrag, q_cdrag_pft, humrel, &
                              ...
                              vbeta23, hist_id, indexveg, indexlai, indexlai0, index, kjit, cim)
...
!!xchen add VPD                                                                 
    ...
    !!WRITE (numout,*) ' VPD before calculation: ', VPD(:)                        
    ! VPD is needed in kPa                                                      
    VPD(:) = VPD(:)/10.0                                                        
    !!WRITE (numout,*) ' VPD after calculation: ', VPD(:)  
    !!CALL qsatcalc (kjpindex, t2m, pb, qsatt)                                    
    air_relhum(:) = &                                                           
      ( qsurf(:) * pb(:) / (Tetens_1+qsurf(:)* Tetens_2) ) / &                  
      ( qsatt(:)*pb(:) / (Tetens_1+qsatt(:)*Tetens_2 ) )                        
    !!WRITE(numout, *) 'air_relhum=', air_relhum(:)                            
    SVP(:) = 610.7*10**(7.5*(t2m(:)-273.15)/(237.3+t2m(:)-273.15))              
    SVP(:) = SVP(:) * (1-air_relhum(:))                                         
    !!WRITE(numout, *) 'SVP=',SVP(:)                         
    DO jv = 1,nvm                                                               
      var_gsmean(:,jv) = gsmean(:,jv)/VPD(:)                                    
    ENDDO                                                                       
!!xchen  
  ...
  !! 6. Combine the alpha and beta coefficients
    ...
    DO jv = 1,nvm
       DO ji = 1, kjpindex
         ...
         ENDIF
       ENDDO
       var_temp2(:,jv) = swdown(:)*W_to_mol*RG_to_PAR
    ...
    CALL xios_orchidee_send_field("FAPAR",var_temp1)                            
    CALL xios_orchidee_send_field("PAR_xc",0.7*var_temp2)                           
    CALL xios_orchidee_send_field("LUE",var_temp3)                              
    CALL xios_orchidee_send_field("PC",var_temp4)
    CALL xios_orchidee_send_field("Gc",var_gsmean)
    CALL xios_orchidee_send_field("Gs",gsmean)       
...
SUBROUTINE diffuco_trans_co2 (kjpindex, lalo, swdown, pb, qsurf, q2m, t2m, temp_growth, rau, u, v, q_cdrag, q_cdrag_pft,  humrel, &
                                assim_param, Ca, &
                                veget, veget_max, lai, qsintveg, qsintmax, vbeta3, vbeta3pot, rveget, rstruct, &
                                cimean, gsmean, gpp, vbeta23, hist_id, indexveg, indexlai, indexlai0, index, kjit, cim)
...
! N. de Noblet - 2006/06 
    ...
    !
#ifdef STRICT_CHECK 
    CALL check_lai_vs_assim(kjpindex, lalo, veget_max, lai, vcmax, "diffuco_trans_co2")
#endif
    !
    ! @addtogroup Photosynthesis
    ! @{   
...
!in precision, but it will affect the general performance
      ...
      !!xchen
      !!DO ji=1,kjpindex 
      !!  WRITE (numout,*) ' swdown in diffuco: ', swdown(ji)
      !!  IF (swdown(ji) .GT. 300) THEN
      !!    water_lim(:) = 5*humrel(:,jv)
      !!  ELSE
          water_lim(:) = humrel(:,jv)
     !!   ENDIF
     !!   WRITE (numout,*) ' swdown in diffuco: ', swdown(ji)                       
...
!!xchen
     CALL xios_orchidee_send_field("fVPD",fvpd)                                    
