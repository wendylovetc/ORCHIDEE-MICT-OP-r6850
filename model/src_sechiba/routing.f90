...
!                  & (precip(ig)+reinfiltration(ig)) )
             ...
             DO jv=2,nvm
                IF ( veget_max(ig,jv) .GT. 0 ) THEN
                    IF ( ok_LAIdev(jv) .AND. (vegstress(ig,jv) .LT. irrig_threshold(jv)) ) THEN
                        IF (irrig_drip) THEN
                            irrig_netereq(ig) = irrig_netereq(ig) + MIN( irrig_dosmax, ( irrigated(ig) * irrig_fulfill(jv) * &
                        ...
                        ELSE !flooding
                            irrig_netereq(ig) = irrig_netereq(ig) + MIN( irrig_dosmax, irrigated(ig) * &
...
#ifdef STRICT_CHECK
       IF (ANY(basin_inbxid(1:nb_basin) < 0)) THEN 
          CALL ipslerr_p(3, 'routing_basins', 'basin_inbxid cannot have negative values.', 'Those will be later used as index in an array.', '')
       ENDIF
#endif
