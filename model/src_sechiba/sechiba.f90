...
!$OMP THREADPRIVATE(indexveg)
  INTEGER(i_std), ALLOCATABLE, SAVE, DIMENSION (:) :: indexlai       !! indexing array for the 3D fields of vegetation for nlai+1
!$OMP THREADPRIVATE(indexlai)
  INTEGER(i_std), ALLOCATABLE, SAVE, DIMENSION (:) :: indexlai0      !! indexing array for the 3D fields of vegetation for nlai
!$OMP THREADPRIVATE(indexlai0)
...
!@chy it should be declared here.@xchen
!$OMP THREADPRIVATE(lai)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: lai_cl1            !! @xchen Surface foliaire
!$OMP THREADPRIVATE(lai_cl1)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: lai_cl2            !! @xchen Surface foliaire
!$OMP THREADPRIVATE(lai_cl2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: lai_cl3            !! @xchen Surface foliaire
!$OMP THREADPRIVATE(lai_cl3)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: lai_cl4            !! @xchen Surface foliaire
!$OMP THREADPRIVATE(lai_cl4)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: laitop            !! @xchen
!$OMP THREADPRIVATE(laish)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: laidow            !! @xchen
!! xchen add vpd for gpp simulation in Amazon
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)  :: VPD              !! Vapor Pressure Deficit (kPa)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)  :: ave_VPD              !! Vapor Pressure Deficit (kPa)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: vcmax_cl1            !! @xchen Surface foliaire
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: vcmax_cl2            !! @xchen Surface foliaire
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: vcmax_cl3            !! @xchen Surface foliaire
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)  :: vcmax_cl4            !! @xchen Surface foliaire
...
!gmjc top 5 layer grassland soil moisture for grazing
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)    :: tmc_topgrass
!$OMP THREADPRIVATE(tmc_topgrass) 
!end gmjc

  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: humcste_use
!$OMP THREADPRIVATE(humcste_use) 
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: altmax
...
!! 0.4 Local variables
    ...
    REAL(r_std), DIMENSION(kjpindex,nslm,nvm)                :: soilmoist_pft 
    REAL(r_std), DIMENSION(kjpindex,nvm)                     :: veget_max_bg !! No nobio. All placed in bareground pft
!_ ================================================================================================================================
    ...
    CALL slowproc_initialize (kjit,     kjpij,          kjpindex,       date0,          &      
                        ...
                        t2m,                                                            &            
                        deadleaf_cover, assim_param,    lai,                            &
!!@xchen
                        ...
                        rest_id_stom,   hist_id_stom,   tot_bare_soil,  fraclut,        &
                        nwdfraclut,                                                     &
                        hist_id_stom_IPCC, co2_flux,    fco2_lu,                        &
                        temp_growth,                                                    &
                        soilc_total,    thawed_humidity, depth_organic_soil,   heat_Zimov, f_rot_sech,altmax)
...
!end gmjc
    ...
    IF (hydrol_cwrr) THEN
       ...
       IF (printlev >=4) THEN
           ...
           WRITE(numout,*) 'soilmoist(1,1)', soilmoist(1,1)
           WRITE(numout,*) 'soilmoist_pft(1,1,:)', soilmoist_pft(1,1,:)
           WRITE(numout,*) 'soilmoist(1,nslm)', soilmoist(1,nslm)
           WRITE(numout,*) 'soilmoist_pft(1,nslm,:)', soilmoist_pft(1,nslm,:)
       ENDIF
       CALL thermosoil_initialize (kjit,  kjpindex, lalo,neighbours,resolution,contfrac, rest_id, veget_max_bg, &
            ...
            mc_layh,        mcl_layh,           soilmoist,   mc_layh_pft, mcl_layh_pft, soilmoist_pft, njsc, &
            frac_snow_veg,  frac_snow_nobio,    totfrac_nobio,     &
            snowdz,         snowrho,            snowtemp,          &
            lambda_snow,    cgrnd_snow,         dgrnd_snow,       pb)
    ELSE
       CALL thermosoilc_initialize (kjit,  kjpindex, lalo, rest_id, veget_max_bg, &
...
!@chy: these lines are not needed. To be declared as "out", it must be passed as
!parameters of sechiba_main, but it is not the case here.           
    !REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out)       :: lai_cl1           !! Land Cover Change CO2 flux 
    !REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (put)       :: lai_cl2           !! Land Cover Change CO2 flux 
    !REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out)       :: lai_cl3           !! Land Cover Change CO2 flux 
    !REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out)       :: lai_cl4           !! Land Cover Change CO2 flux 
    !!REAL(r_std),DIMENSION (kjpindex), INTENT (out)            :: dry_index       !! ratio of 
...
!    REAL(r_std), DIMENSION(kjpindex)                         :: temp_transpot_agr !! potential transpiration by crops need irrigation
    ...
    !!@xchen use var_temp to add FAPAR, PAR_xc, LUE and PC in Wu's paper        
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp1                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp2                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp3                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp4 
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp5                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp6  
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp10                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp9                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp8                       
    REAL(r_std),DIMENSION (kjpindex,nvm)     :: var_temp7                     
...
!_ ================================================================================================================================
    ...
    CALL diffuco_main (kjit, kjpindex, index, indexveg, indexlai, indexlai0, u, v, &
         & zlev, z0m, z0h, roughheight, roughheight_pft, temp_sol, temp_sol_pft, temp_air, temp_growth, rau, tq_cdrag, tq_cdrag_pft, &
...
!!@xchen
    ...
    IF ( .NOT. hydrol_cwrr ) THEN
       ...
       CALL hydrolc_main (kjit, kjpindex, index, indexveg, &
            & temp_sol_new, floodout, runoff, drainage, frac_nobio, totfrac_nobio, contfrac, vevapwet, veget, veget_max,&
...
!            & humrel, vegstress, drysoil_frac, evapot, evapot_corr, evap_bare_lim, evap_bare_lim_pft, flood_frac, flood_res, &
            ...
            & soilflxresid, mc_layh, mcl_layh, soilmoist, mc_layh_s, mcl_layh_s, &
...
!soilcap_pft and temp_sol_new_pft when explicitsnow activated
    ...
    CALL condveg_main (kjit, kjpindex, index,&
           ...
           temp_air, pb, u, v, lai, &
           drysoil_frac, height, snowdz, snowrho, &
           emis, albedo, frac_snow_veg, frac_snow_nobio, z0m, z0h, roughheight, roughheight_pft, &
           rest_id, hist_id, hist2_id)

    !! 7. Compute soil thermodynamics
    CALL vegetation_nobio_to_bg(veget_max, veget_max_bg)
    IF (hydrol_cwrr) THEN      
       !!!!! transfer  mc_layh_s,      mcl_layh_s, soilmoist_s
       !!!!!       to  mc_layh_pft,    mcl_layh_pft, soilmoist_pft
       DO jv = 1,nvm
            mc_layh_pft(:,:,jv) = mc_layh_s(:,:,pref_soil_veg(jv))
            mcl_layh_pft(:,:,jv) = mcl_layh_s(:,:,pref_soil_veg(jv))
            soilmoist_pft(:,:,jv) = soilmoist(:,:)
       ENDDO
!       WRITE(numout,*) 'xuhui: before thermosoil_main'
...
!       irrigation(:) = zero
               ...
               DO jv = 2,nvm
                   IF ( veget_max(ji,jv) .GT. 0 ) THEN
                       ...
                       IF (tempfrac .LE. 0) THEN
                           tempfrac = 0
                       ENDIF
                       IF ( ok_LAIdev(jv) .AND. (vegstress(ji,jv) .LT. irrig_threshold(jv))  ) THEN
                           IF (irrig_drip) THEN
                                  temp_irrig_need = temp_irrig_need + irrig_frac(ji) * &
                                                    & MIN( irrig_dosmax, irrig_fulfill(jv) * MAX( zero, &
    !                                                & transpot(ji,jv)*tempfrac + evapot_corr(ji)*(1-tempfrac) - precip_rain(ji) ) ) &
                                                    & transpot(ji,jv)*tempfrac + evapot(ji)*(1-tempfrac) - precip_rain(ji) ) ) &
                                                    & * veget_max(ji,jv)
    ...
    CALL slowproc_main (kjit, kjpij, kjpindex, date0, &
         ...
         assim_param, &
         lai,&
!!@xchen lai_cl1-4 of leaf age class 1-4
         lai_cl1, lai_cl2,lai_cl3,lai_cl4,laitop,laidow,VPD,ave_VPD,&
         vcmax_cl1, vcmax_cl2,vcmax_cl3,vcmax_cl4,&
         frac_age, height, veget, frac_nobio, veget_max, totfrac_nobio, qsintmax, &
         rest_id, hist_id, hist2_id, rest_id_stom, hist_id_stom, hist_id_stom_IPCC, &
         co2_flux, fco2_lu, temp_growth,&
         swdown, evapot_corr, & !added for crops, xuhui
...
!end gmjc
    ...
    !!@xchen add PC_wu
    !!@xchen add FAPAR=(1-p1)-(1-p2)*exp(-0.5*lai/coszang); p1=0.05 and p2=0 for tower site Santarem km67
    !!umol photons/m^2/si
    !!lue:0.0088176, 0.02162689,0.034436,0.0211652 for lai_cl1-4                                                         
    var_temp1(:,:) = zero                                                       
    var_temp2(:,:) = zero                                                       
    var_temp3(:,:) = zero                                                       
    var_temp4(:,:) = zero
    ...
    DO jv = 1,nvm                                                               
       DO ji = 1, kjpindex
          IF ( coszang(ji) .NE. 0 ) THEN
             var_temp1(ji,jv) = 1-0.05-exp(-0.5*lai(ji,jv)/coszang(ji))                  
          ENDIF
       ENDDO
       !!var_temp2(:,jv) = 0.0088176*lai_cl1(:,jv)+0.02162689*lai_cl2(:,jv)+0.034436*lai_cl3(:,jv)+0.0211652*lai_cl4(:,jv)
       var_temp2(:,jv) = 0.0288176*lai_cl1(:,jv)+0.034436*lai_cl2(:,jv)+0.0211652*lai_cl3(:,jv)+0.00211652*lai_cl4(:,jv)  
       var_temp5(:,jv) = vcmax_cl1(:,jv)/1210.432*lai_cl1(:,jv)+vcmax_cl2(:,jv)/1146.755*lai_cl2(:,jv)+ &
                         & vcmax_cl3(:,jv)/1083.079*lai_cl3(:,jv)+vcmax_cl4(:,jv)/1102.139*lai_cl4(:,jv)
 
       !!WHERE (var_temp2(:,jv) .GT. zero .AND. var_temp1(:,jv) .GT. zero)      
       var_temp3(:,jv) = lai_cl1(:,jv)+lai_cl2(:,jv)+lai_cl3(:,jv)+lai_cl4(:,jv)                        
       DO ji = 1, kjpindex
          IF (var_temp3(ji,jv) .NE. 0) THEN
               var_temp4(ji,jv) =var_temp1(ji,jv)*var_temp2(ji,jv)/var_temp3(ji,jv)
               var_temp6(ji,jv) =var_temp1(ji,jv)*var_temp5(ji,jv)/var_temp3(ji,jv)
          ENDIF
       ENDDO       
       var_temp7(:,jv) = vcmax_cl1(:,jv)/1210.432*swdown(:)*W_to_mol*RG_to_PAR*var_temp1(:,jv)
       var_temp8(:,jv) = vcmax_cl2(:,jv)/1146.755*swdown(:)*W_to_mol*RG_to_PAR*var_temp1(:,jv) 
       var_temp9(:,jv) = vcmax_cl3(:,jv)/1083.079*swdown(:)*W_to_mol*RG_to_PAR*var_temp1(:,jv)
       var_temp10(:,jv) = vcmax_cl4(:,jv)/1102.139*swdown(:)*W_to_mol*RG_to_PAR*var_temp1(:,jv) 
       !!var_temp4(:,jv) = var_temp1(:,jv)*var_temp2(:,jv)                                              
       !!var_temp3(:,jv) = var_temp2(:,jv)*var_temp1(:,jv)                      
       !!ENDWHERE                                                               
    ENDDO    
    CALL xios_orchidee_send_field("PC_wu",var_temp4)
    CALL xios_orchidee_send_field("PC_xc",2.5*var_temp6)                            
    CALL xios_orchidee_send_field("GPP_CL1",var_temp7) 
    CALL xios_orchidee_send_field("GPP_CL2",var_temp8)
    CALL xios_orchidee_send_field("GPP_CL3",var_temp9)
    CALL xios_orchidee_send_field("GPP_CL4",var_temp10)
    CALL xios_orchidee_send_field("GPP_xc",var_temp7+var_temp8+var_temp9+var_temp10)      
    CALL histwrite_p(hist_id, 'gpp_cl1', kjit, var_temp7, kjpindex*nvm, indexveg)
    CALL histwrite_p(hist_id, 'gpp_cl2', kjit, var_temp8, kjpindex*nvm, indexveg)
    CALL histwrite_p(hist_id, 'gpp_cl3', kjit, var_temp9, kjpindex*nvm, indexveg)
    CALL histwrite_p(hist_id, 'gpp_cl4', kjit, var_temp10, kjpindex*nvm, indexveg)
    CALL histwrite_p(hist_id, 'gpp_xc', kjit, var_temp7+var_temp8+var_temp9+var_temp10, kjpindex*nvm, indexveg)
    ...
    ENDDO 
    !!CALL xios_orchidee_send_field("water_profit",precip_rain-histvar*one_day/dt_sechiba-vevapp*one_day/dt_sechiba)
    !!CALL xios_orchidee_send_field("MCWD",evapot_corr-histvar*one_day/dt_sechiba-vevapp*one_day/dt_sechiba)
    CALL xios_orchidee_send_field("PET",evapot_corr/dt_sechiba)
    ...
    CALL xios_orchidee_send_field("vevapnu_pft",vevapnu_pft/dt_sechiba)
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       !CALL histwrite_p(hist_id, 'lai_cl1', kjit, lai_cl1, kjpindex*nvm, indexveg)
       !CALL histwrite_p(hist_id, 'lai_cl2', kjit, lai_cl2, kjpindex*nvm, indexveg)
       !CALL histwrite_p(hist_id, 'lai_cl3', kjit, lai_cl3, kjpindex*nvm, indexveg)
       !CALL histwrite_p(hist_id, 'lai_cl4', kjit, lai_cl4, kjpindex*nvm, indexveg)
    ...
    ELSE
       ...
       CALL histwrite_p(hist_id, 'nobiofrac', kjit, frac_nobio, kjpindex*nnobio, indexnobio)
       CALL histwrite_p(hist_id, 'lai', kjit, lai, kjpindex*nvm, indexveg)
       IF (.NOT. ok_explicitsnow) CALL histwrite_p(hist_id, 'Qf', kjit, fusion, kjpindex, index)
...
!                        snowcap,        snowgrain,  drysoil_frac, evap_bare_lim, evap_bare_lim_pft)
    ...
    CALL condveg_finalize (kjit, kjpindex, rest_id, z0m, z0h, roughheight, roughheight_pft)
...
!! 1. Initialize variables 
    ...
    ALLOCATE (indexlai0(kjpindex*(nlai)),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for indexlai0','','')
...
!!@chy: lai_cl1/2/3/4 has to be allocated here. @xchen
    ALLOCATE (lai_cl1(kjpindex,nvm),stat=ier)                                       
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for lai_cl1','','')  
    lai_cl1(:,:)=undef_sechiba
    ALLOCATE (lai_cl2(kjpindex,nvm),stat=ier)                                   
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for lai_cl2','','')  
    lai_cl2(:,:)=undef_sechiba
    ALLOCATE (lai_cl3(kjpindex,nvm),stat=ier)                                   
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for lai_cl3','','')  
    lai_cl3(:,:)=undef_sechiba 
    ALLOCATE (lai_cl4(kjpindex,nvm),stat=ier)                                   
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for lai_cl4','','')  
    lai_cl4(:,:)=undef_sechiba 
    ALLOCATE (VPD(kjpindex),stat=ier)                                    
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for VPD','','')
    VPD(:)=undef_sechiba 
    ALLOCATE (ave_VPD(kjpindex),stat=ier)                                           
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for ave_VPD','','')  
    ave_VPD(:)=undef_sechiba    
    ALLOCATE (laitop(kjpindex,nvm),stat=ier)                                    
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for laitop','','')
    laitop(:,:)=undef_sechiba                                                   
    ALLOCATE (laidow(kjpindex,nvm),stat=ier)                                     
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for laidow','','')
    laidow(:,:)=undef_sechiba  
    ALLOCATE (vcmax_cl1(kjpindex,nvm),stat=ier)                                   
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for vcmax_cl1','','')
    vcmax_cl1(:,:)=undef_sechiba  
    ALLOCATE (vcmax_cl2(kjpindex,nvm),stat=ier)                                 
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for vcmax_cl2','','')
    vcmax_cl2(:,:)=undef_sechiba 
    ALLOCATE (vcmax_cl3(kjpindex,nvm),stat=ier)                                 
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for vcmax_cl3','','')
    vcmax_cl3(:,:)=undef_sechiba 
    ALLOCATE (vcmax_cl4(kjpindex,nvm),stat=ier)                                 
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for vcmax_cl4','','')
    vcmax_cl4(:,:)=undef_sechiba 
...
! xuhui: +
    ...
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in f_rot_sech allocation. We stop. we need kjpindex words = ',kjpindex
       STOP 'sechiba_init'
    END IF
    f_rot_sech(:) = .FALSE.
...
!end gmjc
    ...
    ALLOCATE (humcste_use(kjpindex,nvm),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for humcste_use','','')
...
!    ALLOCATE (soilmoist_pft(kjpindex, nslm, nvm),stat=ier)
!    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for soilmoist_pft','','')
    ...
    DO ji = 1, kjpindex
       !
       DO jv = 1, nlai
          indexlai0((jv-1)*kjpindex + ji) = INDEX(ji) + (jv-1)*kjpij + offset_omp - offset_mpi
       ENDDO
       !
       DO jv = 1, nlai+1
       ...
       DO jv = 1, nslm
          indexnslm((jv-1)*kjpindex + ji) = INDEX(ji) + (jv-1)*kjpij + offset_omp - offset_mpi
       ENDDO
...
!! 2. Deallocate dynamic variables of sechiba
    ...
    IF ( ALLOCATED (indexlai0)) DEALLOCATE (indexlai0)
...
!@chy: these lines below are needed.
!!@xchen
    ...
    IF ( ALLOCATED (VPD)) DEALLOCATE (VPD)   
    IF ( ALLOCATED (ave_VPD)) DEALLOCATE (ave_VPD)                                                          
    IF ( ALLOCATED (laitop)) DEALLOCATE (laitop)                                
    IF ( ALLOCATED (laidow)) DEALLOCATE (laidow) 
    IF ( ALLOCATED (vcmax_cl1)) DEALLOCATE (vcmax_cl1)                              
    IF ( ALLOCATED (vcmax_cl2)) DEALLOCATE (vcmax_cl2)                          
    IF ( ALLOCATED (vcmax_cl3)) DEALLOCATE (vcmax_cl3)                          
    IF ( ALLOCATED (vcmax_cl4)) DEALLOCATE (vcmax_cl4)                          
...
!end gmjc
    IF ( ALLOCATED (humcste_use)) DEALLOCATE (humcste_use)
...
!    IF ( ALLOCATED (soilmoist_pft)) DEALLOCATE (soilmoist_pft)
