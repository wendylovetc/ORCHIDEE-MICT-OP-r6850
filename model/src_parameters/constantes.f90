  ...
  SUBROUTINE activate_sub_models()
    ...
    IF (ok_stomate) THEN
       ...
       IF (use_age_class) THEN
         ...
         !!!! oilpalm, yidi 
         ok_oilpalm = .FALSE.
         CALL getin_p('OK_OILPALM',ok_oilpalm)
         !!!! end oilpalm, yidi
...
!!!!! xuhui
    ...
    !
    !Config Key   = SANDFRACTION_DEFAULT
    !Config Desc  = default fraction of sand
    !Config If    = OK_SECHIBA 
    !Config Def   = 0.4 
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('SANDFRACTION_DEFAULT',sandfraction_default)
    !
    !Config Key   = SILTFRACTION_DEFAULT
    !Config Desc  = default fraction of silt
    !Config If    = OK_SECHIBA 
    !Config Def   = 0.4 
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('SILTFRACTION_DEFAULT',siltfraction_default)


    IF ( ABS(clayfraction_default+sandfraction_default+siltfraction_default-1) > min_sechiba) THEN
       WRITE(numout,*) 'Incoherence found. clayfraction_default=', clayfraction_default, ' sandfraction_default=',&
            sandfraction_default,' siltfraction_default=',siltfraction_default
       CALL ipslerr_p(3,"config_sechiba_parameters",&
            "Inconsistecy between CLAYFRACTION_DEFAULT, SANDFRACTION_DEFAULT and SILTFRACTION_DEFAULT set in run.def",&
            "The sum should be equal 1 but this is not the case.","Modify run.def and restart the model")
    END IF
    !
...
!pss+       
    ...
    IF (TOPM_calcul) CALL topmodel_parameters()
!pss+
...
!! yidi
    !
    !Config Key   = MIN_R 
    !Config Desc  = extrema of root allocation fraction 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.1
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('MIN_R',min_R)
    !
    !Config Key   = MIN_RTOLSR 
    !Config Desc  = extrema of root allocation fraction 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.05
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('MIN_RTOLSR',min_RtoLSR)
    !
    !Config Key   = MAX_RTOLSR 
    !Config Desc  = extrema of root allocation fraction 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.05
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('MAX_RTOLSR',max_RtoLSR)
    !
    !Config Key   = SUP_LTOLSR 
    !Config Desc  = extrema of root allocation fraction 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.05
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('SUP_LTOLSR',sup_LtoLSR)
!! yidi
    !
...
!! yidi
    ...
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.0,0.3 ! xchen=0.0 0.0
    !Config Help  = 
    !Config Units = [-]   
    ...
    !
    !
    !Config Key   = LSR_COEFF
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 100,3,0.6 ! xchen=30
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('LSR_COEFF',lsr_coeff)
    !
    ...
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('RS_COEFF',rs_coeff)
    !
    !Config Key   = FFB_COEFF
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.02, 0.85
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('FFB_COEFF',ffb_coeff)
    !
    !Config Key   = PHY_COEFF
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 0.25, 2.0, 1.0
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('PHY_COEFF',phy_coeff)
    !
    !Config Key   = LEAFTURN_COEFF
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 2.0, 3.0
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('LEAFTURN_COEFF',leafturn_coeff)
    !
    !Config Key   = LEAFTURN_COEFF2
    !Config Desc  = 
    !Config If    = OK_STOMATE 
    !Config Def   = 180.0,1.0,6.0
    !Config Help  = 
    !Config Units = [-]   
    CALL getin_p('LEAFTURN_COEFF2',leafturn_coeff2)
    !
!! yidi
    !-
