...
!$OMP THREADPRIVATE(ok_dgvm)
  LOGICAL :: ok_oilpalm         !! yidi activate oil palm module
!$OMP THREADPRIVATE(ok_oilpalm)
...
!$OMP THREADPRIVATE(alpha_nudge_snow)
  ...
  INTEGER(i_std), PARAMETER :: nphs2 = 40         !! yidi number of photomers (unitless)
  !-
  ...
  INTEGER(i_std)            :: nwp = 2         !! Number of types of wood product pool considered
...
!$OMP THREADPRIVATE(snowcri)
  REAL(r_std), SAVE :: irrig_dosmax = 1.25 !! The maximum irrigation water injected for half-hour (kg.m^{-2}/30min)
...
!$OMP THREADPRIVATE(siltfraction_default)
  REAL(r_std), SAVE :: min_vegfrac = 1E-8           !! Minimal fraction of mesh a vegetation type can occupy (0-1, unitless)
...
!!  REAL(r_std), SAVE :: max_LtoLSR = 0.5            !! Prescribed upper bounds for leaf
  REAL(r_std), SAVE :: max_LtoLSR = 0.9            !!xchen171201 Prescribed upper bounds for leaf 
                                                   !! allocation (0-1, unitless)
!$OMP THREADPRIVATE(max_LtoLSR)
!! yidi
  REAL(r_std), SAVE :: sup_LtoLSR = 0.65            !! Prescribed lower leaf allocation
                                                   !! when lai decrease (0-1, unitless)
!$OMP THREADPRIVATE(sup_LtoLSR)
  REAL(r_std), SAVE :: min_RtoLSR = 0.05            !! Prescribed lower bounds for root 
                                                   !! allocation (0-1, unitless)
!$OMP THREADPRIVATE(min_RtoLSR)
  REAL(r_std), SAVE :: min_R = 0.1            !! Prescribed lower bounds for root 
                                                   !! allocation (0-1, unitless)
!$OMP THREADPRIVATE(min_R)
  REAL(r_std), SAVE :: max_RtoLSR = 0.35            !! Prescribed upper bounds for root 
                                                   !! allocation (0-1, unitless)
!$OMP THREADPRIVATE(max_RtoLSR)
!! yidi
...
!$OMP THREADPRIVATE(allow_forestry_harvest)
  LOGICAL, SAVE :: gluc_use_harvest_biomass        !! Use harvest biomass ?
!$OMP THREADPRIVATE(gluc_use_harvest_biomass)
  LOGICAL, SAVE :: gluc_allow_trans_bioe          !! Transitions to bioenergy ?
!$OMP THREADPRIVATE(gluc_allow_trans_bioe)
...
!$OMP THREADPRIVATE(nagec_herb)
  INTEGER, SAVE :: nagec_bioe1                    !! Number of age classes for bioe1 MTC
!$OMP THREADPRIVATE(nagec_bioe1)
  LOGICAL, SAVE :: gluc_newfrac_guide            !! Use guided new vegetfrac in gross LUC?
!$OMP THREADPRIVATE(gluc_newfrac_guide)
...
!$OMP THREADPRIVATE(leaf_age_crit_tref)
  REAL(r_std), SAVE :: phy_allocmax = 0.25    !! (C)
!$OMP THREADPRIVATE(phy_allocmax)
  REAL(r_std), SAVE, DIMENSION(3) :: leaf_age_crit_coeff = (/ 1.5, 0.75, 10./) !! (unitless)
!$OMP THREADPRIVATE(leaf_age_crit_coeff)
!! yidi
  REAL(r_std), SAVE,  DIMENSION(3) :: lsr_coeff = (/ 95., 3., 0.6/) !! (unitless)
!$OMP THREADPRIVATE(lsr_coeff)
  REAL(r_std), SAVE,  DIMENSION(2) :: rs_coeff = (/ 0.9, 0.9/) !! (unitless)
!$OMP THREADPRIVATE(rs_coeff)
!  REAL(r_std), SAVE :: turn_coeff = 2.0 !! (unitless)
!!$OMP THREADPRIVATE(turn_coeff)
  REAL(r_std), SAVE, DIMENSION(2) :: ffb_coeff = (/ 0.02, 1.5 /) !! (unitless)
!$OMP THREADPRIVATE(2)
  REAL(r_std), SAVE, DIMENSION(3) :: phy_coeff = (/ 0.25, 2.0, 1.0 /) !! (unitless)
!$OMP THREADPRIVATE(2)
  REAL(r_std), SAVE, DIMENSION(2) :: leafturn_coeff = (/ 2.0, 3.0 /) !! (unitless)
!$OMP THREADPRIVATE(2)
  REAL(r_std), SAVE, DIMENSION(3) :: leafturn_coeff2 = (/ 180.0, 1.0, 6.0 /) !! (unitless)
!$OMP THREADPRIVATE(2)
!! yidi
  ...
  REAL(r_std), SAVE,  DIMENSION(2) :: vmax_offset = (/ 0.0, 0.3/) !! yidi minimum leaf efficiency (unitless)
  !REAL(r_std), SAVE :: vmax_offset = 0.0        !! xchen  minimum leaf efficiency (unitless)
  !REAL(r_std), SAVE :: vmax_offset = 0.3        !! minimum leaf efficiency (unitless)
!$OMP THREADPRIVATE(vmax_offset)
  REAL(r_std), SAVE :: leafage_firstmax = 0.001  !! xchen relative leaf age at which efficiency
  !REAL(r_std), SAVE :: leafage_firstmax = 0.03  !! relative leaf age at which efficiency
                                                !! reaches 1 (unitless)
!$OMP THREADPRIVATE(leafage_firstmax)
  REAL(r_std), SAVE :: leafage_lastmax = 0.001    !! xchen relative leaf age at which efficiency
