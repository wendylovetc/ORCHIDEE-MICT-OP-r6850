...
!_ ================================================================================================================================
  ...
  SUBROUTINE control_initialize
    ...
    WRITE(numout,*) "Netcdf restart compression is : ", NC_COMPRESSION_ENABLE
...
!!!! yidi oilpalm    
    !
    !Config Key   = OK_OILPALM
    !Config Desc  = Should we include OILPALM 
    !Config If    = IF  
    !Config Def   = n
    !
    ok_oilpalm = .FALSE.
    ...
    !IF ( ok_oilpalm ) THEN
        !!  Number of Phytomers of oil palm defined by the user
        !Config Key   = NPH
        !Config Desc  = number of NPHs
        !Config If    = OK_OILPALM
        !Config Def   = 40
        !Config Help  = The number of PHYTOMERS in one oilpalm tree
        !Config Units = [-]
        !
        nphs = 40
        CALL getin_p('NPH',nphs)
        IF (printlev>=1) WRITE(numout,*) 'The number of phytomers used by the model is : ', nphs

        !!  Number of leafagecohort of oil palm defined by the user
        !Config Key   = NLC
        !Config Desc  = number of NLCs
        !Config If    = OK_OILPALM
        !Config Def   = 4
        !Config Help  = The number of LEAF IN one PHYTOMERS
        !Config Units = [-]
        !
        !CALL getin_p('NLC',nlc)
        !IF (printlev>=1) WRITE(numout,*) 'The number of leafagecohort used in one phtomers is : ', nlc
       
    !END IF
    IF (printlev>=1) WRITE(numout,*) 'oil palm is activated: ', ok_oilpalm
!!!! yidi oilpalm
    !
    ...
    !
    !Config Key   = STOMATE_OK_OILPALM
    !Config Desc  = Activate OILPALM MODULE?
    !Config If    = OK_STOMATE
    !Config Def   = n
    !Config Help  = set to TRUE if OILPALM is to be activated
    !Config Units = [FLAG]
    !    
    ! yidi
    !ok_oilpalm = .FALSE.
    !CALL getin_p('STOMATE_OK_OILPALM',ok_oilpalm)
    !
...
!!!! yidi  Number of Phytomers of oil palm defined by the user
    !Config Key   = NPH
    !Config Desc  = number of NPHs  
    !Config If    = OK_OILPALM
    !Config Def   = 40
    ...
    !
    !CALL getin_p('NPH',nphs)
    !IF (printlev>=1) WRITE(numout,*) 'The number of phtomers used by the model is : ', nphs
!!!! yidi
