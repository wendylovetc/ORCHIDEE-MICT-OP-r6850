...
!!!!! crops
    ...
    ! the nth of half layer,[1, 2, .....2*nslm]
    INTEGER(i_std)                                                    :: nhl
    ! the upper limit humidity for ith half layer of 22 half layers, 2*nslm
    INTEGER(i_std), DIMENSION(npts, 2*nslm)                                   :: ulhum
    ! the ith layer for the three layers around sowing depth
    INTEGER(i_std)                                                    :: il
    ! uldep is the upper limit of ith half layer of 22 half layers, 2*nslm
    !REAL(r_std), DIMENSION(2*nslm)                                  :: uldep
    REAL(r_std)                                                     :: uldep
    ! bldep is the lower limit of ith half layer of 22 half layers
    REAL(r_std)                                                     :: bldep
    
    ! the upper limit soil temperature for ith half layer of 22 half layers, 2*nslm
    REAL(r_std), DIMENSION(npts, 2*nslm)                                   :: ultsoil
            ...
            DO nhl = 1, 2*nslm
              IF ((ldep(ipft, il) .GE. uldep) .AND. (ldep(ipft,il) .LT. bldep)) THEN 
                  IF (nhl < 2*nslm) THEN
                     ...
                     ! linear interpolation
                  ELSE
                     soilhum_cm(:, ipft, il) = soilhum(:, nslm) 
                  ENDIF
              ENDIF
  ...
  !! 5. fluxes from litter to carbon pools and respiration
    ...
    DO l = 1, nlevs !Loop over litter levels (above and below ground)
       DO m = 1,nvm !Loop over PFTs
          ...
          IF(l==iabove) THEN
             DO k=1, npts
               IF (fuel_nlitt_total_pft(k,m,istructural,icarbon).GT.min_stomate) THEN
               ...
               IF (ABS(diff_frac(k)).GT.min_stomate .AND. fuel_nlitt_total_pft(k,m,istructural,icarbon).GT.min_stomate) THEN
                   ! simply divided even fraction to each fuel class, that is 1/4 of buffer
                   fuel_1hr(k,m,istructural,icarbon) = fuel_1hr(k,m,istructural,icarbon) + fuel_1hr(k,m,istructural,icarbon) &
                                                         * diff_frac(k)/fuel_nlitt_total_pft(k,m,istructural,icarbon)
                   fuel_10hr(k,m,istructural,icarbon) = fuel_10hr(k,m,istructural,icarbon) + fuel_10hr(k,m,istructural,icarbon) &
                                                         * diff_frac(k)/fuel_nlitt_total_pft(k,m,istructural,icarbon)
                   fuel_100hr(k,m,istructural,icarbon) = fuel_100hr(k,m,istructural,icarbon) + fuel_100hr(k,m,istructural,icarbon) &
                                                         * diff_frac(k)/fuel_nlitt_total_pft(k,m,istructural,icarbon)
                   fuel_1000hr(k,m,istructural,icarbon) = fuel_1000hr(k,m,istructural,icarbon) + fuel_1000hr(k,m,istructural,icarbon) &
                                                           * diff_frac(k)/fuel_nlitt_total_pft(k,m,istructural,icarbon)
                   fuel_nlitt_total_pft(k,m,istructural,icarbon) = fuel_1hr(k,m,istructural,icarbon) + &
                        fuel_10hr(k,m,istructural,icarbon) + fuel_100hr(k,m,istructural,icarbon) + & 
                        fuel_1000hr(k,m,istructural,icarbon)
      ...
      ! time step and avoid some constructions that could create bug during future developments.
          ...
          IF(l==iabove) THEN
             DO k=1, npts
               IF (fuel_nlitt_total_pft(k,m,imetabolic,icarbon).gt.min_stomate) THEN
               ...
               IF(ABS(diff_frac(k)).GT.min_stomate .AND. fuel_nlitt_total_pft(k,m,imetabolic,icarbon).gt.min_stomate) THEN
                   ! simply divided even fraction to each fuel class, that is 1/4 of buffer
                   fuel_1hr(k,m,imetabolic,icarbon) = fuel_1hr(k,m,imetabolic,icarbon) + fuel_1hr(k,m,imetabolic,icarbon)&
                                                       * diff_frac(k)/fuel_nlitt_total_pft(k,m,imetabolic,icarbon)
                   fuel_10hr(k,m,imetabolic,icarbon) = fuel_10hr(k,m,imetabolic,icarbon) + fuel_10hr(k,m,imetabolic,icarbon)&
                                                       * diff_frac(k)/fuel_nlitt_total_pft(k,m,imetabolic,icarbon)
                   fuel_100hr(k,m,imetabolic,icarbon) = fuel_100hr(k,m,imetabolic,icarbon) + fuel_100hr(k,m,imetabolic,icarbon)&
                                                       * diff_frac(k)/fuel_nlitt_total_pft(k,m,imetabolic,icarbon)
                   fuel_1000hr(k,m,imetabolic,icarbon) = fuel_1000hr(k,m,imetabolic,icarbon) + fuel_1000hr(k,m,imetabolic,icarbon)&
                                                       * diff_frac(k)/fuel_nlitt_total_pft(k,m,imetabolic,icarbon)
                   fuel_nlitt_total_pft(k,m,imetabolic,icarbon) = fuel_1hr(k,m,imetabolic,icarbon) + &
                        fuel_10hr(k,m,imetabolic,icarbon) + fuel_100hr(k,m,imetabolic,icarbon) + &
                        fuel_1000hr(k,m,imetabolic,icarbon)
