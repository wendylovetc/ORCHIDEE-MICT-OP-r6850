...
MODULE stomate_permafrost_soilcarbon
  ...
  ! Constants
  REAL(r_std), PARAMETER                                :: MICROACTEM_Q10 = 2.0
...
!! 0. Variable and parameter declaration    
  ...
  !$OMP THREADPRIVATE(no_pfrost_decomp)
...
!!       END WHERE
    ...
    IF ( .NOT. soilc_isspinup ) THEN
       ...
       CALL xios_orchidee_send_field ('febul', febul)
       CALL xios_orchidee_send_field ('flupmt',  flupmt)
  ...
  !$OMP THREADPRIVATE(use_fixed_cryoturbation_depth)
    ...
    IF (firstcall) THEN
	  ...
	  alpha_p(:,:,:)=0.
          ...
          IF (ier.NE.0) THEN
             WRITE (numout,*) ' error in mu_soil_rev allocation. We stop. We need ',kjpindex,' fois ',nvm,' words = '&
              & , kjpindex*nvm
             STOP 'deep_carbcycle'
          END IF
	  mu_soil_rev(:,:)=0.
          ...
          DO ip = 1, kjpindex
             DO iv = 1, nvm
                IF ( cryoturb_location(ip,iv) .OR. bioturb_location(ip,iv) )THEN 
                   ! 1. calculate the total soil carbon
                   DO il = 1, ndeep
                   ...
                   deepC_a(ip,1,iv) = (deepC_a(ip,1,iv)+mu_soil_rev(ip,iv)*beta_a(ip,1,iv)) / (1.+mu_soil_rev(ip,iv)*(1.-alpha_a(ip,1,iv)))
                   deepC_s(ip,1,iv) = (deepC_s(ip,1,iv)+mu_soil_rev(ip,iv)*beta_s(ip,1,iv)) / (1.+mu_soil_rev(ip,iv)*(1.-alpha_s(ip,1,iv)))
                   deepC_p(ip,1,iv) = (deepC_p(ip,1,iv)+mu_soil_rev(ip,iv)*beta_p(ip,1,iv)) / (1.+mu_soil_rev(ip,iv)*(1.-alpha_p(ip,1,iv)))
                   ...
                   ! 3. recalculate the total soil carbon
                   DO il = 1, ndeep
                   ...
                   IF ( altC_a_old(ip,iv) > min_stomate .AND. (ABS(altC_a(ip,iv)-altC_a_old(ip,iv))/altC_a_old(ip,iv).GT. min_stomate) ) THEN
                      WRITE (numout,*) 'DZ warn: cryoturbate: total C not conserved','ip=',ip,'iv=',iv,'A,diff=',altC_a(ip,iv),altC_a_old(ip,iv),altC_a(ip,iv)-altC_a_old(ip,iv),(altC_a(ip,iv)-altC_a_old(ip,iv))/altC_a_old(ip,iv)
                      CALL ipslerr_p (3,'cryoturbate','','','')
                   ENDIF

                   IF ( altC_s_old(ip,iv) > min_stomate .AND. (ABS(altC_s(ip,iv)-altC_s_old(ip,iv))/altC_s_old(ip,iv).GT. min_stomate) ) THEN
                      WRITE (numout,*) 'DZ warn: cryoturbate: total C not conserved','ip=',ip,'iv=',iv,'S,diff=',altC_s(ip,iv),altC_s_old(ip,iv),altC_s(ip,iv)-altC_s_old(ip,iv),(altC_s(ip,iv)-altC_s_old(ip,iv))/altC_s_old(ip,iv)
                      CALL ipslerr_p (3,'cryoturbate','','','')
                   ENDIF

                   IF ( altC_p_old(ip,iv) > min_stomate .AND. (ABS(altC_p(ip,iv)-altC_p_old(ip,iv))/altC_p_old(ip,iv).GT. min_stomate) ) THEN
                      WRITE (numout,*) 'DZ warn: cryoturbate: total C not conserved','ip=',ip,'iv=',iv,'P,diff=',altC_p(ip,iv),altC_p_old(ip,iv),altC_p(ip,iv)-altC_p_old(ip,iv),(altC_p(ip,iv)-altC_p_old(ip,iv))/altC_p_old(ip,iv)
                      CALL ipslerr_p (3,'cryoturbate','','','')
                   ENDIF
...
!_
!================================================================================================================================     
  FUNCTION microactem ( temp_kelvin, frozen_respiration_func, moist_in, i_ind, j_ind, k_ind, zi_soil ) RESULT ( fbact )
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), DIMENSION(i_ind, j_ind, k_ind), INTENT(in)  :: moist_in
    REAL(r_std), DIMENSION(i_ind, j_ind, k_ind), INTENT(in)  :: temp_kelvin
    ...
    REAL(r_std), DIMENSION(i_ind, j_ind, k_ind)              :: fbact
    REAL(r_std), DIMENSION(i_ind, j_ind, k_ind)              :: tempfunc_result
    INTEGER(i_std), PARAMETER                         :: ntconfun = 7
    REAL(r_std), DIMENSION(ntconfun)                  :: tconfun
    REAL(r_std), DIMENSION(ntconfun)                  :: tauconfun
    INTEGER                                           :: itz
    INTEGER                                           :: ii, ij, ik
    REAL(r_std), DIMENSION(i_ind, j_ind, k_ind)              :: moistfunc_result
  ...
  !$OMP THREADPRIVATE(is_firstcall)

    REAL(r_std), DIMENSION(j_ind)                     :: tmp_exp

    IF (is_firstcall) THEN
        !
        !Config Key   = depth_modifier 
        !Config Desc  = e-folding depth of turnover rates,following Koven et
        !Config DEsc    al.,2013,Biogeosciences. A very large value means no depth modification 
        !Config Def   = 1.E6 
        !Config If    = 
        !Config Help  = 
        !Config         
    ...
    CASE(0) ! this is the standard ORCHIDEE state
       ...
       tempfunc_result(:,:,:) = EXP( log(MICROACTEM_Q10) * ( temp_kelvin(:,:,:) - (ZeroCelsius+30.) ) * 0.1 )
       tempfunc_result(:,:,:) = MIN( 1._r_std, tempfunc_result(:,:,:) )

    CASE(1)  ! cutoff respiration when T < -1C
       CALL microactem_cutoff_resp(i_ind,j_ind,k_ind,1., temp_kelvin, tempfunc_result)

    CASE(2)  ! cutoff respiration when T < -3C
       CALL microactem_cutoff_resp(i_ind,j_ind,k_ind,3., temp_kelvin, tempfunc_result)

    CASE(3)  ! q10 = 100 when below zero
       CALL microactem_neg_q10 ( 100., temp_kelvin, tempfunc_result )

    CASE(4)  ! q10 = 1000 when below zero
       CALL microactem_neg_q10 ( 1000., temp_kelvin, tempfunc_result )

    CASE DEFAULT
       CALL ipslerr_p(3,'microactem','frozen respiration func not in list','',frozen_respiration_func)
    ...
    ! Precalculate EXP() due to an expensive cost
    tmp_exp(:) = 1 / EXP(-zi_soil(:)/depth_modifier)

    DO ik = 1,  k_ind
      DO ij = 1, j_ind
        !DIR$ VECTOR ALIGNED
        DO ii = 1, i_ind
          fbact(ii,ij,ik) = stomate_tau/(moistfunc_result(ii,ij,ik) * tempfunc_result(ii,ij,ik)) * tmp_exp(ij)
        ENDDO
      ENDDO
    ENDDO
    ...
    DO ik = 1,nvm
      IF ( (.NOT. natural(ik)) .AND. (.NOT. is_c4(ik)) .AND. (.NOT. pasture(ik)) ) THEN
        fbact(:,:,ik) = fbact(:,:,ik)/flux_tot_coeff(1)
      ELSEIF ( (.NOT. natural(ik)) .AND. is_c4(ik) .AND. (.NOT. pasture(ik)) ) THEN
        fbact(:,:,ik) = fbact(:,:,ik)/flux_tot_coeff(2)
      ENDIF
    ENDDO
...
!!
!================================================================================================================================
!! SUBROUTINE   : microactem_cutoff_resp
!!
!>\BRIEF        This routine calculates the respiration temperature with a cutoff value
!!
!! DESCRIPTION : 
!!
!! RECENT CHANGE(S) : None
...
!_
!================================================================================================================================     
  SUBROUTINE microactem_cutoff_resp (i_ind,j_ind,k_ind, cutoff_temp, temp_kelvin, tempfunc_result)

    ! 0.1 Input vars
    INTEGER(i_std), INTENT(in)                 :: i_ind
    INTEGER(i_std), INTENT(in)                 :: j_ind
    INTEGER(i_std), INTENT(in)                 :: k_ind
    REAL(r_std), INTENT(in)                    :: cutoff_temp ! cutoff temperature (in celcius)
    REAL(r_std), DIMENSION(i_ind,j_ind,k_ind), INTENT(in)  :: temp_kelvin

    ! 0.3 Output vars
    REAL(r_std), DIMENSION(i_ind,j_ind,k_ind), INTENT(out)  :: tempfunc_result

    ! 0.4 Local vars
    REAL(r_std), parameter                     :: q10 = 2.0
    REAL(r_std), DIMENSION(i_ind,j_ind,k_ind)  :: tmp
    REAL(r_std) :: inv_cutoff_temp, tmp_const
    INTEGER(i_std) :: ii,ij,iv ! loop iterators

    ! precalculate values
    inv_cutoff_temp = 1 / cutoff_temp
    tmp_const = EXP( log(MICROACTEM_Q10) * ( ZeroCelsius - (ZeroCelsius+30.) ) / 10. )
    ...
    !DIR$ SIMD
    DO ii=1, i_ind
       IF (temp_kelvin(ii,ij,iv) .GT. ZeroCelsius ) THEN ! normal as above
          tempfunc_result(ii,ij,iv) = EXP( log(MICROACTEM_q10) * ( temp_kelvin(ii,ij,iv) - (ZeroCelsius+30.) ) * 0.1 )
       ELSEIF (temp_kelvin(ii,ij,iv) .GT. ZeroCelsius - cutoff_temp ) THEN ! linear dropoff to zero
          tempfunc_result(ii,ij,iv) = ((temp_kelvin(ii,ij,iv) - (ZeroCelsius - cutoff_temp))* inv_cutoff_temp) * &
                                   tmp_const
       ELSE  ! zero
          tempfunc_result(ii,ij,iv) = EPSILON(0.)
       ENDIF
    ENDDO ! ii
    ENDDO ! ij
    ENDDO ! jv

  END SUBROUTINE microactem_cutoff_resp 

!!
!================================================================================================================================
!! SUBROUTINE   : microactem_neg_q10
!!
!>\BRIEF        This routine calculates the temperature function with a different q10 when temperature is negative
!!
!! DESCRIPTION : 
!!
!! RECENT CHANGE(S) : None
...
!_
!================================================================================================================================     
  SUBROUTINE microactem_neg_q10 ( neg_q10, temp_kelvin, tempfunc_result )

    ! 0.1 Input vars
    REAL(r_std), INTENT(in)             :: neg_q10  ! use when temp is below 0: Negative q10
    REAL(r_std), DIMENSION(:,:,:), INTENT(in)  :: temp_kelvin

    ! 0.3 Output vars
    REAL(r_std), DIMENSION(SIZE(temp_kelvin,DIM=1), SIZE(temp_kelvin,DIM=2), SIZE(temp_kelvin,DIM=3)), INTENT(out) :: tempfunc_result

    ! 0.4 Local vars
    REAL(r_std), DIMENSION(SIZE(temp_kelvin,DIM=1), SIZE(temp_kelvin,DIM=2), SIZE(temp_kelvin,DIM=3)) :: tmp


    tmp = temp_kelvin(:,:,:) - ZeroCelsius

    WHERE (temp_kelvin(:,:,:) .GT. ZeroCelsius ) ! normal as above
       tempfunc_result(:,:,:) = EXP( log(MICROACTEM_Q10) * ( tmp+30. ) * 0.1 )
    ELSEWHERE 
       tempfunc_result(:,:,:) = EXP( log(neg_q10) * tmp * 0.1 ) * &
            EXP( log(MICROACTEM_Q10) * -30. * 0.1 )
    ENDWHERE

  END SUBROUTINE microactem_neg_q10
