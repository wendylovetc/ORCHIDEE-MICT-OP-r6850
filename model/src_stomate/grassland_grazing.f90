...
!        print *,  'min_grazing', min_grazing
      ...
      ! determines the number of days for which animal in the barn (year_length_in_days - compt_ugb(:))and 
            ...
            !equal to mauto_C3 and mauto_C4
              WHERE ((ok_ugb(:,j) .EQ. 0))
                ...
                WHERE ( import_yield(:,j) .GT. 0.0 )
                  extra_feed(:,j)  = (year_length_in_days - compt_ugb(:,j)) * 18 * sr_ugb(:,j) 
      ...
      ! crop feed 
        IF (f_autogestion .EQ. 3) THEN
          WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0))
            extra_feed(:,mgraze_C3)  = (year_length_in_days - compt_ugb(:,mgraze_C3)) * 18 *sr_ugb(:,mgraze_C3)
          ...
          WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0))
            extra_feed(:,mgraze_C4)  = (year_length_in_days - compt_ugb(:,mgraze_C4)) * 18 *sr_ugb(:,mgraze_C4)
      ...
      ! 5: auto graze for PFT m_grazed with grazing litter during winter for LGM
        ...
        IF ((f_postauto .EQ. 1) .OR. (f_postauto .EQ. 2)) THEN
          ...
          WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0) .AND. (sr_ugb(:,mgraze_C3) .GT. 0.0))
            extra_feed(:,mgraze_C3)  = (year_length_in_days - compt_ugb(:,mgraze_C3)) * 18.0*sr_ugb(:,mgraze_C3)
            ...
            consump(:,mgraze_C3) = (year_length_in_days - compt_ugb(:,mgraze_C3)) * 18.0 * nb_ani(:,mgraze_C3)
            ...
            ! of the past 5 years
            add_nb_ani(:,mgraze_C3) = outside_food(:,mgraze_C3)/ (18.0 * year_length_in_days) * 0.2
          ...
          WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0) .AND. (sr_ugb(:,mgraze_C4).GT. 0.0))
            extra_feed(:,mgraze_C4)  = (year_length_in_days - compt_ugb(:,mgraze_C4)) *18.0*sr_ugb(:,mgraze_C4)
            ! total yield of las year (kg DM/m^2 total grassland)
            amount_yield(:,mgraze_C4) = import_yield(:,mgraze_C4) *(1-grazed_frac(:,mgraze_C4)) * 0.85
            ! total animal indoor consumption of last year (kg DM/m^2 total grassland)       
            consump(:,mgraze_C4) = (year_length_in_days - compt_ugb(:,mgraze_C4)) * 18.0 *nb_ani(:,mgraze_C4)
            ! food surplus (outside_food > 0) or deficit (outside_food < 0)
            outside_food(:,mgraze_C4) = amount_yield(:,mgraze_C4)-consump(:,mgraze_C4)
            ! farmers' decision of buy (add_nb_ani > 0) or sell (add_nb_ani < 0) animals 
            add_nb_ani(:,mgraze_C4) = outside_food(:,mgraze_C4)/ (18.0 *year_length_in_days) * 0.2
            !! New animal density for total grassland
        ...
        IF (f_postauto .EQ. 5) THEN
          WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0) .AND. &
            ...
            extra_feed(:,mgraze_C3)  = (year_length_in_days - compt_ugb(:,mgraze_C3)) * 18.0*sr_ugb(:,mgraze_C3)
            amount_yield(:,mgraze_C3) = import_yield(:,mgraze_C3) * (1-grazed_frac(:,mgraze_C3)) * 0.85
            consump(:,mgraze_C3) = 0.0 !(year_length_in_days - compt_ugb(:,mgraze_C3)) * 18.0 * nb_ani(:,mgraze_C3)
            outside_food(:,mgraze_C3) = 0.0 !amount_yield(:,mgraze_C3)-consump(:,mgraze_C3)
            add_nb_ani(:,mgraze_C3) = 0.0 !outside_food(:,mgraze_C3)/ (18.0 * year_length_in_days) * 0.2
            nb_ani(:,mgraze_C3)=nb_ani(:,mgraze_C3)+add_nb_ani(:,mgraze_C3)
            WHERE (sr_ugb(:,mgraze_C3) .GT. 0.0)
          ...
          WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0) .AND. (sr_ugb(:,mgraze_C4).GT. 0.0))
            extra_feed(:,mgraze_C4)  = (year_length_in_days - compt_ugb(:,mgraze_C4)) *18.0*sr_ugb(:,mgraze_C4)
            amount_yield(:,mgraze_C4) = import_yield(:,mgraze_C4) *(1-grazed_frac(:,mgraze_C4)) * 0.85
            consump(:,mgraze_C4) = 0.0 !(year_length_in_days - compt_ugb(:,mgraze_C4)) * 18.0 *nb_ani(:,mgraze_C4)
            outside_food(:,mgraze_C4) = 0.0 !amount_yield(:,mgraze_C4)-consump(:,mgraze_C4)
            add_nb_ani(:,mgraze_C4) = 0.0 !outside_food(:,mgraze_C4)/ (18.0 *year_length_in_days) * 0.2
            nb_ani(:,mgraze_C4)=nb_ani(:,mgraze_C4)+add_nb_ani(:,mgraze_C4)
            WHERE (sr_ugb(:,mgraze_C4) .GT. 0.0)
...
!ENDJCMODIF
  ...
  ENDIF ! f_autogestion = 5
    ...
    CALL Euler_funct (dt,grazingn, grazingnsum)       
    CALL Euler_funct (dt, grazingc, grazingcsum)  
      ...
      DO i=1,npts
        ...
        ! During the grazing period, wich begins at tanimal and finishes at tanimal+danimal
           IF((tjulian .GE. tanimal(i,j,h)) .AND. &
            ...
            CALL Euler_funct(dt, intake(i,j), intakesum(i,j))
            CALL Euler_funct(dt, intake_animal(i,j), intake_animalsum(i,j))
            temp(i,j)=intake(i,j)*(n(i,j)+fn(i,j))
            CALL Euler_funct(dt, temp(i,j), intakensum(i,j))
          ENDIF
          h= h+1
        ENDDO
      ENDDO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Milk_Animal(&
    ...
    REAL(r_std), DIMENSION(npts,nvm)  :: tmp_milk
    ...
    CALL Euler_funct(dt, milk , milksum)
    CALL Euler_funct(dt, milkc, milkcsum)
    CALL Euler_funct(dt, milkn, milknsum)

    milkndaily  = milknsum  - milknsumprev 
    tmp_milk = nel*intake_animal*nanimaltot
    CALL Euler_funct(dt, tmp_milk, nelgrazingsum)
    CALL Euler_funct(dt, milkanimal, milkanimalsum)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Respiration_Methane(&
    ...
    CALL Euler_funct(dt, ranimal, ranimalsum)
    CALL Euler_funct(dt, methane, Methanesum)
    CALL Euler_funct(dt, methane_ani, Methane_aniSum) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Urine_Faeces(&
    ...
    CALL Euler_funct(dt, urinen, urinensum)
    urinendaily  = urinensum  - urinensumprev

    CALL Euler_funct(dt, faecesn, faecesnsum)
    faecesndaily  = faecesnsum  - faecesnsumprev
    ...
    CALL Euler_funct(dt, urinec, urinecsum)
    CALL Euler_funct(dt, faecesc, faecescsum)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Animaux_main_dynamic(&
    ...
    n_year : IF (new_year .EQV. .TRUE. ) THEN
        ...
        ! Then the model simulates the same grasslang plot with animals. Stocking rate "S"
        ! is incremented at each optimization step. For each stocking rate, the program
        ! determines the number of days for which animals are in the barn (year_length_in_days - compt_ugb(:))
        ...
        IF(f_nonlimitant .EQ. 0) THEN
            ...
            IF (f_postauto .EQ. 1) THEN
                ...
                WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0))
                  ...
                  ! total animal indoor consumption of last year (kg DM/m^2 total grassland)                 
                   consump(:,mgraze_C3) = (year_length_in_days - compt_ugb(:,mgraze_C3)) * &
...
!gmjc postauto=5
            ...
            IF (f_postauto .EQ. 5) THEN
                WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0) .AND. &
                      (sr_ugb(:,mgraze_C3) .GT. 0.0))
                   extra_feed(:,mgraze_C3)  = (year_length_in_days - compt_ugb(:,mgraze_C3)) * &
                  ...
                  ! total animal indoor consumption of last year (kg DM/m^2 total grassland)
                   consump(:,mgraze_C3) = 0.0
                   !(year_length_in_days - compt_ugb(:,mgraze_C3)) * 18.0 * nb_ani(:,mgraze_C3)
                  ! food surplus (outside_food > 0) or deficit (outside_food < 0)
                   outside_food(:,mgraze_C3) = 0.0
                   !amount_yield(:,mgraze_C3)-consump(:,mgraze_C3)
                  ! farmers' decision of buy (add_nb_ani > 0) or sell (add_nb_ani < 0) animals
                   add_nb_ani(:,mgraze_C3) = 0.0
                   !outside_food(:,mgraze_C3)/ (18.0 * year_length_in_days) * 0.2
                  !! New animal density for total grassland
                  nb_ani(:,mgraze_C3)=nb_ani(:,mgraze_C3)+add_nb_ani(:,mgraze_C3)
                  !! New fraction of grazed grassland in total grassland (keep the same stocking rate)
                  WHERE (sr_ugb(:,mgraze_C3) .GT. 0.0)
...
!                  ENDWHERE
                ...
                WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0) .AND. (sr_ugb(:,mgraze_C4).GT. 0.0))
                   ...
                   extra_feed(:,mgraze_C4)  = (year_length_in_days - compt_ugb(:,mgraze_C4)) * &
...
!end gmjc
    ...
    DO j=2,nvm
     ...
     !84 est year_length_in_days moins la durée de gestation(280j) 
          ...
          ENDIF
        ELSE
           IF (tempTjulian+year_length_in_days-tcalving(i,j) .GE. 84 .and. &
                tempTjulian+year_length_in_days-tcalving(i,j) .LE. year_length_in_days) THEN
            gestation(i,j)=1
          ENDIF
          IF (year_length_in_days-(tcalving(i,j)-tempTjulian).LT. age_sortie_calf(i,j)+1) THEN
            calf(i,j)=1
          ENDIF
        ENDIF
      ENDDO
      ...
      IF (f_autogestion.EQ.2) THEN
            DO i=1,npts
               IF (nanimaltot_prec(i,j).EQ.0.AND.&
                   ...
                   ELSE
                      CALL estime_weightcalf(year_length_in_days+tjulian-tcalving(i,j),Wcalfborn(i,j),Wanimalcalf(i,j))
                   ENDIF
...
!----------------------------
    ...
    ELSEIF(type_animal.EQ.2) THEN ! Suckler cows
        ...
        !   (grazing_intake_complementation is never called for calves variables     
          ...
          CALL Euler_funct (dt,DMIcalfanimal, DMIcalfanimalsum)
          CALL Euler_funct (dt,DMIcalf, DMIcalfsum)
          tmp_var = DMIcalf*(n+fn)
          CALL Euler_funct (dt,tmp_var,DMIcalfnsum)
  ...
  ! la production de lait au pas de temps d'avant pour le calcul de la complémentation
   ...
   !-------------------------------------
    ...
    CALL Euler_funct (dt,intake_animal, intake_animalsum)
...
!------------------------
    ...
    tmp_var = intake*nel
    CALL Euler_funct (dt,tmp_var,nelgrazingsum)
  ...
  ! Suckler Calf
   ...
   SUBROUTINE intake_capacity_calves(&
    ...
    DO j=2,nvm
      ...
      ELSE 
         IF (dsevrage(i,j).GT.year_length_in_days) THEN
            IF (tjulian.GT.dsevrage(i,j)-year_length_in_days.AND.tjulian.LT.tcalving(i,j)) THEN
               ICcalf(i,j)=0.0345*(wanimalcalf(i,j)**0.9)
            ELSE  
...
!    ENDDO
  ...
  ENDIF
   ...
   CALL Euler_funct(dt, DMIcow, DMIcowsum)
   CALL Euler_funct(dt, DMIcowanimal, DMIcowanimalsum)

   temp(:,:,1)=DMIcow(:,:,1)*(n(:,:)+fn(:,:))
   temp(:,:,2)=DMIcow(:,:,2)*(n(:,:)+fn(:,:))
   
   CALL Euler_funct(dt, temp, DMIcowNsum)
  ...
  ENDDO     
   ...
   CALL Euler_funct (dt,forage_complementc,forage_complementcsum)
   CALL Euler_funct (dt,forage_complementn,forage_complementnsum)
  ...
  SUBROUTINE Milk_Animal_cow(         &
    ...
    DO j=2,nvm
      DO i=1,npts
        ! Week of lactation for cows
            ...
            ! si tjulian est inférieur à tcalving on considere le velage de l'annee precedente   
                nWeeklact(i,j) = CEILING((tjulian-(REAL(tcalving(i,j))-year_length_in_days)+1)/7)             
            END IF        
       
            
            dsevrage(i,j)=tcalving(i,j)+tsevrage(i,j)
            IF (dsevrage(i,j) > year_length_in_days) THEN
               dsevrage(i,j)=dsevrage(i,j)-year_length_in_days
            ENDIF   
      ...
      ENDDO      
        ...
        CALL Euler_funct(dt, MPcow , MPcowsum)
        CALL Euler_funct(dt, MPcowC, MPcowCsum)
        CALL Euler_funct(dt, MPcowN, MPcowNsum)
        CALL Euler_funct(dt, MPcow2, MPcow2sum)   
        CALL Euler_funct(dt, MilkKG, milkanimalsum)
    ...
    DO j=2,nvm  
      ...
      IF (nanimaltot(i,j).ne.0) THEN
        ...
        IF (jourdepuisvelage .lt. 0) THEN 
            jourdepuisvelage=year_length_in_days+jourdepuisvelage
        ENDIF   
        ...
        ELSEWHERE      
          !NEPgest=26.3*exp(-0.0184*(year_length_in_days-jourdepuisvelage))
          NEPgest=3.70*7.12*exp(-0.0184*(year_length_in_days-jourdepuisvelage))
        ENDwhere 
        ...
        CALL Euler_funct (dt, NEBcow_calc(i,j), wanimalcow(i,j))
        ...
        CALL Euler_funct (dt, NEBcow_calc(i,j), BCS(i,j))
  ...
  CALL Euler_funct(dt, NEG_calc, wanimalcalf)     
  ...
  INTEGER                                     :: j
    ...
    ! no grazing period      
       ...
       NEIheifer_W=0.
    ENDWHERE
   CALL Euler_funct (dt, NEIheifer_W, wanimalcow)
...
SUBROUTINE calcul_perte_etat(npts,tjulian,BCScow,MPwmax,tcalving,PEmax) 
  ...
  ELSEWHERE    
 ! si tjulian est inférieur à tcalving on considere le velage de l'annee precedente   
       nWeeklact = CEILING((tjulian-(REAL((tcalving)-year_length_in_days)))/7+1)                
  ENDWHERE
...
!----------------------------------------
  ...
  SUBROUTINE Potentiel_dairy_d(npts,tjulian,nweekLact,nweekGest,MPwcow2max,MPwcow2)
    ...
    !------------------------------
        ...
        ! si tjulian est inférieur à tcalving on considere le velage de l'annee precedente   
            nWeeklact = CEILING((tjulian-(REAL((tcalving)-year_length_in_days)))/7+1)
            nWeekGest = CEILING((tjulian-80-(REAL((tcalving)-year_length_in_days)))/7+1)                 
        ENDWHERE 
