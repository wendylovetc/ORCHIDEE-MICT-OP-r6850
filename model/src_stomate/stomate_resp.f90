...
!_ ================================================================================================================================
 ...
 !! 1. Initializations
    ...
    CALL calculate_root_temp(npts, z_soil, rprof, stempdiag, t_root) 
...
!$                         ENDIF
       ...
       ENDDO ! Loop over # plant parts
          ...
          IF ((j .GE. 2) .AND. (j .LE. 7)) THEN
             WRITE(numout,*) 'yd: resp1,nvm=',j,' coeff_maint(:,j,:) after resp maint',coeff_maint(:,j,:)
          ENDIF
...
!    WRITE(numout,*) 'lai after stomate_resp: ',lai(1,12:14)
     IF (printlev>=3) WRITE(numout,*) 'Leaving respiration##############################'

  END SUBROUTINE maint_respiration

!!
!================================================================================================================================
!! SUBROUTINE   : calculate_root_temp
!!
!>\BRIEF        Calculate root temperature as the convolution of root and soil temperature profiles
!!
!! DESCRIPTION : 
!!
!! RECENT CHANGE(S) : None
...
!_
!================================================================================================================================     
  SUBROUTINE calculate_root_temp(npts, z_soil, rprof, stempdiag, t_root )

    ! 0.1 Input variables
    INTEGER(i_std), INTENT(in)                       :: npts      !! Domain size - number of grid cells (unitless)
    REAL(r_std), DIMENSION(0:nslm), INTENT(in)       :: z_soil    !! Variable to store depth of the different soil layers (m) 0:nslm

    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: rprof     !! PFT root depth as calculated in stomate.f90 from parameter npts,nvm
                                                                  !! humcste which is root profile for different PFTs 
                                                                  !! in slowproc.f90 (m)
    REAL(r_std), DIMENSION(npts,nslm), INTENT (in)   :: stempdiag !! Soil temperature of each soil layer (K) npts,nslm

    ! 0.3 Ouput arguments
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)    :: t_root    !! PFT root temperature (convolution of root and soil npts,nvm 
                                                                  !! temperature profiles) (K)
    ...
    REAL(r_std), DIMENSION(npts,nvm,0:nslm)          :: exp_zsoil_rprof !! Temp array to precalculate

    INTEGER(i_std) :: j,l !! Iterators

    ! precalculate: EXP is a very expensive call
    DO j = 2,nvm ! Loop over # PFTs

       DO l = 0, nslm ! Loop over # soil layers

          !DIR$ VECTOR ALIGNED
          exp_zsoil_rprof(:,j,l) =  EXP( -z_soil(l)/rprof(:,j) )
    ...
    DO j = 2,nvm ! Loop over # PFTs
       ...
       !  \endlatexonly
       !DIR$ VECTOR ALIGNED
       rpc(:) = un / ( un - exp_zsoil_rprof(:,j,nslm) ) 
       ...
       DO l = 1, nslm ! Loop over # soil layers
          ...
          t_root(:,j) = &
               t_root(:,j) + stempdiag(:,l) * rpc(:) * &
                ( exp_zsoil_rprof(:,j,l-1) - exp_zsoil_rprof(:,j,l))

       ENDDO ! Loop over # soil layers

    ENDDO ! Loop over # PFTs

  END SUBROUTINE calculate_root_temp
