...
!! yidi
!! no bm_sapl added to bm_phytomer PHYbm FFBbm, bm_sapl to bm_sapling first
              !! IF (ok_oilpalm) THEN
              !!   IF (is_oilpalm(j)) THEN
              !!      IF (biomass(i,j,:,:) .GT. min_stomate) THEN
              !!          bm_phytomer(:,j,1)= bm_phytomer(:,j,1) + &
              !!                 (biomass(j,isapabove,icarbon) * 
              !!                  PHYbm(:,j) / biomass(:,j,isapabove,icarbon)
              !!  PHYbm(:,j)=PHYbm(:,j)+ &
              !!                 biomass(j,isapabove,icarbon)*  PHYbm(:,j) / biomass(:,j,isapabove,icarbon)
              !!    ENDIF
              !! 1ENDIF
!!yidi
              leaf_frac(i,j,:) = zero
