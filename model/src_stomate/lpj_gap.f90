...
!end gmjc
    ...
    INTEGER(i_std)                                          :: i, j,k,m,p              !! Indices
          ...
          IF ( .NOT.  lpj_gap_const_mort  ) THEN
             IF (ok_oilpalm) THEN
                IF (is_oilpalm(j)) THEN
                   WRITE(numout,*) 'yd: lpj0: LPJ_GAP_CONST_MORT=no'
                   WRITE(numout,*) 'yd: lpj0: residence_time',residence_time(j)
                ENDIF
             ENDIF
...
!ENDJCMODIF
          ...
          ELSE  ! .NOT. lpj_gap_const_mort
             IF (ok_oilpalm) THEN
                IF (is_oilpalm(j)) THEN
                   WRITE(numout,*) 'yd: lpj0: LPJ_GAP_CONST_MORT=y'
                   WRITE(numout,*) 'yd: lpj0: residence_time',residence_time(j)
                ENDIF
             ENDIF
...
!             WHERE ( PFTpresent(:,j) .AND. ( npp_longterm(:,j) .LE. min_stomate ) )
             WHERE ( PFTpresent(:,j) .AND. ( npp_longterm(:,j) .LT. (npp_longterm_init-un) ) )
                mortality(:,j) = un
...
!! yidi
          IF (ok_oilpalm) THEN
             IF (is_oilpalm(j)) THEN
                WRITE(numout,*) 'yd: lpj1 nvm=',j,' mortality',mortality(:,j)
                WRITE(numout,*) 'yd: lpj1 nvm=',j,' bm_ffb',FFBbm(:,j)
                WRITE(numout,*) 'yd: lpj1 nvm=',j,' bm_PHY',PHYbm(:,j)
                DO p = 1, nphs
                   WHERE ( PFTpresent(:,j) )
                          ! phytomer_age(i,j,p) = phytomer_age(i,j,p) - phytomer_age(i,j,p) * mortality(i,j)
                            bm_phytomer(:,j,p) = bm_phytomer(:,j,p) - bm_phytomer(:,j,p) * mortality(:,j)
                            bm_FFB(:,j,p) = bm_FFB(:,j,p) - bm_FFB(:,j,p) * mortality(:,j)
                   ENDWHERE
                ENDDO
                WHERE ( PFTpresent(:,j) )
                         PHYbm(:,j) = PHYbm(:,j) - PHYbm(:,j) * mortality(:,j)
                         FFBbm(:,j) = FFBbm(:,j) - FFBbm(:,j) * mortality(:,j)
                ENDWHERE
                WRITE(numout,*) 'yd: lpj2 nvm=',j,' bm_ffb',FFBbm(:,j)
                WRITE(numout,*) 'yd: lpj2 nvm=',j,' bm_PHY',PHYbm(:,j)
             ENDIF
          ENDIF
         ! IF (ok_oilpalm) THEN
         !    IF (is_oilpalm(j)) THEN
         ...
         !                DO p = 1, nphs
         !                 ! phytomer_age(i,j,p) = phytomer_age(i,j,p) - phytomer_age(i,j,p) * mortality(i,j)
         !                   bm_phytomer(i,j,p) = bm_phytomer(i,j,p) - bm_phytomer(i,j,p) * mortality(i,j)
         !                   bm_FFB(i,j,p) = bm_FFB(i,j,p) - bm_FFB(i,j,p) * mortality(i,j)
         !                ENDDO
         !                PHYbm(i,j) = PHYbm(i,j) - PHYbm(i,j) * mortality(i,j)
         !                FFBbm(i,j) = FFBbm(i,j) - FFBbm(i,j) * mortality(i,j)
         !             ENDIF
         !          ENDDO
         !    ENDIF
         ! ENDIF
...
! JC comment: though natural(j) is defined before do we still need pasture here?
!          IF ( ok_dgvm ) THEN
          IF ( ok_dgvm .AND. natural(j) .AND. .NOT. pasture(j)) THEN
