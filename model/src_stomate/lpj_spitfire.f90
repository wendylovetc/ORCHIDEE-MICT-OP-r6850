...
!! yidi
    ...
    INTEGER :: j,x,m,k,n,i,ivm,ipts,p
...
!! yidi
          IF (ok_oilpalm) THEN
             IF (is_oilpalm(j)) THEN
                 DO p = 1, nphs
                      !phytomer_age(:,j,p) = phytomer_age(:,j,p) -tree_kill_frac(:) * phytomer_age(:,j,p)
                      bm_phytomer(:,j,p) = bm_phytomer(:,j,p) - tree_kill_frac(:) * bm_phytomer(:,j,p)
                      bm_FFB(:,j,p) = bm_FFB(:,j,p) - tree_kill_frac(:) * bm_FFB(:,j,p)
                 ENDDO
                 PHYbm(:,j) = PHYbm(:,j)- tree_kill_frac(:) * PHYbm(:,j)
                 FFBbm(:,j) = FFBbm(:,j)- tree_kill_frac(:) * FFBbm(:,j)
             ENDIF
          ENDIF
!! yidi
...
!debuging only
    ...
    CALL xios_orchidee_send_field("D_AREA_BURNT",d_area_burnt)     
    CALL xios_orchidee_send_field("FIREFRAC_SPITFIRE",fire_frac) 
