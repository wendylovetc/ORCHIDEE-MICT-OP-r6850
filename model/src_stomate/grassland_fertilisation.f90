...
CONTAINS
  ...
  SUBROUTINE fertilisation_pas_temps(&
    ...
    REAL(r_std), DIMENSION(npts,nvm)        , INTENT(inout)     :: Fert_on
    ! variables locales
    INTEGER :: i,j
      ...
      DO i= 1, npts
        IF (fertcount(i,j) .GT. 0) THEN
          ...
          fcOrganicFertmetabolic(i,j)  =  &
             fvgcmetabolic * 15.0 * fnfert_resultat1(i,j) + &
             fkacmetabolic * 15.0 * fnfert_resultat2(i,j) + &
             fmistcmetabolic * c2Nsolidmanure * fnfert_resultat3(i,j)
          fcOrganicFertstruct(i,j)  =  &
             (1.0 - fvgcmetabolic) * 15.0 * fnfert_resultat1(i,j) + &
             (1.0 - fkacmetabolic) * 15.0 * fnfert_resultat2(i,j) + &
