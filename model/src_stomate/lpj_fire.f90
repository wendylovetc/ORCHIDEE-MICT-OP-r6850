...
!! yidi
          IF (ok_oilpalm) THEN
             IF (is_oilpalm(j)) THEN
                PHYbm(:,j)= (un-fire_disturb(:)) * PHYbm(:,j)   
                FFBbm(:,j)= (un-fire_disturb(:)) * FFBbm(:,j)
                    DO p = 1, nphs
                       bm_phytomer(:,j,p) =(un-fire_disturb(:)) * bm_phytomer(:,j,p)
                       bm_FFB(:,j,p) = (un-fire_disturb(:)) * bm_FFB(:,j,p) 
                    ENDDO                   
             ENDIF
          ENDIF
!! yidi
