...
!    REAL(r_std), DIMENSION(npts,nvm,nphs,nleafages), INTENT(inout) ::phytomer_lfage   !! leafage of phytomers (days)
...
!! yidi
    ...
    INTEGER(i_std)                                            :: j,m,p           !! Indices       (unitless)
...
!! yidi
             IF (ok_oilpalm) THEN
                IF ( is_oilpalm(j)) THEN
                   DO p = 1, nphs
                      WHERE ( was_killed(:) )
                         phytomer_age(:,j,p) = zero 
                         bm_phytomer(:,j,p) = zero
                         bm_FFB(:,j,p) = zero
                      ENDWHERE
                   ENDDO
                   WHERE ( was_killed(:) )
