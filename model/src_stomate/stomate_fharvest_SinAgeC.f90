...
MODULE stomate_fharvest_SinAgeC
  ...
  PRIVATE
  PUBLIC fharvest_SinAgeC
  
CONTAINS

! ================================================================================================================================
! Note: [2016-04-20]
! This is a simple duplication of gross_glcchange_SinAgeC_fh, to be called in
! StomateLpj to allow forstry harvest being handled before the land cover change.
! Some duplicate histwrite_p are commented.
!_ ================================================================================================================================
  SUBROUTINE fharvest_SinAgeC (npts, dt_days, harvest_matrix,newvegfrac,   &
               fuelfrac, &
    ...
    REAL(r_std), DIMENSION(npts,nwp), INTENT(inout)            :: convflux         !! release during first year following land cover
                                                                             !! change
    REAL(r_std), DIMENSION(npts,nwp), INTENT(inout)            :: cflux_prod10     !! total annual release from the 10 year-turnover
                                                                             !! pool @tex ($gC m^{-2}$) @endtex
    REAL(r_std), DIMENSION(npts,nwp), INTENT(inout)            :: cflux_prod100    !! total annual release from the 100 year-
...
!! yidi
    ...
    WRITE(numout,*) 'Entering fharvest_SinAgeC'
    glcc_harvest(:,:) = zero
    glccReal_tmp(:,:) = zero

    CALL SinAgeC_fh_firstday(npts,veget_max,newvegfrac,harvest_matrix,   &
                          glccSecondShift,glccPrimaryShift,glccNetLCC,&
  ...
  END SUBROUTINE fharvest_SinAgeC

! ================================================================================================================================
! Note: [2016-04-20]
! This is a simple duplication of gross_glcchange_SinAgeC_fh, to hanle the case that
! only forestry harvest is included.
!_ ================================================================================================================================
  ...
  SUBROUTINE SinAgeC_fh_firstday(npts,veget_max_org,newvegfrac,harvest_matrix, &
                          glccSecondShift,glccPrimaryShift,glccNetLCC,&
                          glccReal,glcc_pft,glcc_pftmtc,IncreDeficit, &
                          Deficit_pf2yf_final, Deficit_sf2yf_final,   &
                          pf2yf_compen_sf2yf, sf2yf_compen_pf2yf)
  ...
  END SUBROUTINE SinAgeC_fh_firstday

END MODULE stomate_fharvest_SinAgeC
