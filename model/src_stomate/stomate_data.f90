...
!! yidi
  INTEGER(i_std), ALLOCATABLE, SAVE, DIMENSION(:) :: horiphytomer_index
!$OMP THREADPRIVATE(horiphytomer_index)
!! yidi
...
!!!$      ENDIF
       ...
       ELSE
          ...
          IF (ok_LAIdev(j)) THEN
              IF ( natural(j) ) THEN
                 ...
                 CALL ipslerr_p(3, 'data','configuration error','both ok_LAIdev and natural in some PFT(s)','')
              ELSE
       ...
       IF (.NOT. ok_dgvm .AND. use_age_class) THEN
       ...
       !! yidi
       ! 15 maximum lifetime of phytomer and harvest days
       IF ( ( printlev >= 2 ) .AND. ( phytomeragecrit(j) .NE. undef ) ) &
            WRITE(numout,*) '       critical phytomer age (d): (::phytomeragecrit(j))', phytomeragecrit(j)
       IF ( ( printlev >= 2 ) .AND. ( ffbharvestagecrit(j) .NE. undef ) ) &
            WRITE(numout,*) '       critical ffb age (d): (::ffbharvestagecrit(j))', ffbharvestagecrit(j)
       ! 16 time constant for phytomer discretisation (days)
       !

       phytomer_timecst(j) = phytomeragecrit(j) / REAL( nphs,r_std )

       IF ( printlev >= 2 ) &
            WRITE(numout,*) '       time constant for phytomer age discretisation (d): (::phytomer_timecst(j))', &
            phytomer_timecst(j)
       !! yidi
       !
...
! dgvmjc
       IF ( printlev >= 1 ) &
            WRITE(numout,*) '       PASTURE: (::pasture(j))', pasture(j)
! end dgvmjc
       !
