...
!end gmjc
    ...
    REAL(r_std), DIMENSION(npts), INTENT(inout)           :: VPD_week         !! xchen daily VPD
    REAL(r_std), DIMENSION(npts), INTENT(in)        :: swdown_daily           !! xchen daily swdown    
    REAL(r_std), DIMENSION(npts), INTENT(inout)        :: swdown_week            !! xchen weekly swdown
...
!_ =================================================================================================================================
    ...
    IF ( firstcall_season ) THEN
       ...
       ENDIF
!!xchen
       IF ( ABS( SUM( VPD_week(:) ) ) .LT. min_stomate ) THEN                   
                                                                                
          ! in this case, set them to today's temperature                       
          WRITE(numout,*) 'Warning! We have to initialize the ''weekly'' VPD.'
          VPD_week(:) = VPD_daily(:)                                            
                                                                                
       ENDIF
       IF ( ABS( SUM( swdown_week(:) ) ) .LT. min_stomate ) THEN                
                                                                                
          ! in this case, set them to today's temperature                       
          WRITE(numout,*) 'Warning! We have to initialize the ''weekly'' swdown.'
          swdown_week(:) = swdown_daily(:)                                      
       ENDIF                                                                         
...
!end gmjc
    ...
    ENDWHERE
!!xchen weekly VPD
    VPD_week = ( VPD_week * ( tau_t2m_week - dt ) + &                           
         VPD_daily * dt ) / tau_t2m_week                                        
                                                                                
    WHERE ( ABS(VPD_week(:)) .LT. EPSILON(zero) )                               
       VPD_week(:) = zero                                                       
    ENDWHERE    
    swdown_week = ( swdown_week * ( tau_t2m_week - dt ) + &                     
         swdown_daily * dt ) / tau_t2m_week                                     
                                                                                
    WHERE ( ABS(swdown_week(:)) .LT. EPSILON(zero) )                            
       swdown_week(:) = zero                                                    
    ENDWHERE  
!gmjc
    !
