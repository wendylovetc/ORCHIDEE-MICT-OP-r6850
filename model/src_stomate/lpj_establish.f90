...
!end gmjc
    ...
    INTEGER(i_std)                            :: i,j,k,l,m,ipts,ipart,ilit,p     !! indices (unitless)       
!_ ================================================================================================================================
    ...
    estab_rate_max_grass(:) = zero
    sumfpc_wood(:) = zero
    fpc_nat(:,:) = zero
    estab_rate_max_tree(:) = zero
...
!ENDJCMODIF
       ...
       DO j = 2,nvm ! Loop over # PFTs
          ...
          IF ( .NOT. is_tree(j) .AND. natural(j) .AND. .NOT. pasture(j)) THEN
       ...
       factor(:) = min_stomate
       DO j = 2,nvm ! Loop over # PFTs
          IF ( natural(j) .AND. .NOT.is_tree(j) .AND. .NOT. pasture(j)) & 
...
!ENDJCMODIF
       ...
       DO j = 2,nvm ! Loop over # PFTs
          ...
          IF ( natural(j) .AND. .NOT. pasture(j)) THEN ! only for natural PFTs
...
!ENDJCMODIF
          ...
          ELSEIF ( natural(j) .AND. .NOT.is_tree(j) .AND. .NOT. pasture(j)) THEN 
          ...
          DO k = 1, nparts ! Loop over # litter tissues (nparts=8); see 'stomate_constants.f90'
             WHERE(d_ind(:,j).GT.min_stomate.AND.total_bm_c(:).GT.min_stomate.AND.veget_cov_max(:,j).GT.min_stomate)
                ...
                total_bm_sapl(:,icarbon) = total_bm_sapl(:,icarbon) + & 
                     bm_sapl(j,k,icarbon) * d_ind(:,j) / veget_cov_max(:,j)
          ...
          DO k = 1, nparts ! Loop over # litter tissues
             ...
             DO l=1, npts
                IF( d_ind(l,j).GT.min_stomate .AND. &
                   ...
                   veget_cov_max(l,j).GT.min_stomate) THEN

                   bm_new(l) = d_ind(l,j) * bm_sapl(j,k,icarbon) / veget_cov_max(l,j)
                   ...
                   IF ((veget_cov_max_tree(l) .GT. 0.1) .AND. (veget_cov_max(l,j) .LT. veget_cov_max_tree(l)/nbtree) ) THEN

                      bm_non(l) = MIN( biomass(l,j,k,icarbon)+bm_to_litter(l,j,k,icarbon), &
                        (ind(l,j)+d_ind(l,j))*mortality(l,j) * bm_sapl(j,k,icarbon)/veget_cov_max(l,j) )
                   ...
                   ELSE
                      ...
                      bm_non(l) = MIN( bm_to_litter(l,j,k,icarbon), &
                        (ind(l,j)+d_ind(l,j))*mortality(l,j) * bm_sapl(j,k,icarbon)/veget_cov_max(l,j) )
             ...
             DO ipts=1, npts
                IF (d_ind(ipts,j).GT.min_stomate.AND.total_bm_c(ipts).GT.min_stomate) THEN
                   ...
                   IF ((veget_cov_max_tree(ipts) .GT. 0.1) .AND. (veget_cov_max(ipts,j) .LT. veget_cov_max_tree(ipts)/nbtree) ) THEN
 ...
 !! yidi
          IF (ok_oilpalm) THEN
             IF (is_oilpalm(j)) THEN
                !! newly established saplings the biomass is added to phytomer
                !but not FFB

                !! phytomer_age is changed with dt
                !WHERE (d_ind(:,j)*bm_sapl(j,isapabove,icarbon) .GT. min_stomate )          
                !   phytomer_age(:,j,1) = phytomer_age(:,j,1) * bm_phytomer(:,j,1) / &
                !        (bm_phytomer(:,j,1) + d_ind(:,j) * bm_sapl(j,isapabove,icarbon) )
                !ENDWHERE       
                ! bm_sapl / (biomass-biomass_old) 
                !! PHYbm(:,j)/bm sapabove or bm_phytomer(:,j,1)/bm_sapabove
                !bm_phytomer(:,j,1)= bm_phytomer(:,j,1) + d_ind(:,j) * bm_sapl(j,isapabove,icarbon) * &
                !                PHYbm(:,j) / biomass_old(:,j,isapabove,icarbon)
                !PHYbm(:,j)=PHYbm(:,j)+ d_ind(:,j) * bm_sapl(j,isapabove,icarbon) * &
                !                PHYbm(:,j) / biomass_old(:,j,isapabove,icarbon)   
                !FFBbm(:,j)=FFBbm(:,j)+d_ind(:,j) * bm_sapl(j,isapabove,icarbon)* &
                !                FFBbm(:,j) / biomass_old(:,j,isapabove,icarbon)
                WHERE (biomass_old(:,j,isapabove,icarbon) .GT. min_stomate) 
                   bm_phytomer(:,j,1)= bm_phytomer(:,j,1) + &
                                (biomass(:,j,isapabove,icarbon) - biomass_old(:,j,isapabove,icarbon)) * &
                                PHYbm(:,j) / biomass_old(:,j,isapabove,icarbon)
                   PHYbm(:,j)=PHYbm(:,j)+ &
                                (biomass(:,j,isapabove,icarbon) - biomass_old(:,j,isapabove,icarbon)) * &
                                PHYbm(:,j) / biomass_old(:,j,isapabove,icarbon)   
          ...
          ENDIF
!! yidi
          !! 4.9 Update age and onumber of individuals
