  ...
  !couplage avec ORCHIDEE (pas de temps, boucle sur les grid-cells, etc.)

  !Cette routine calcule les densites de flux de CH4 pour un wetlands dont la water table depth est constante dans le temps et situee à la surface du sol
  ...
  !Dans cette routine, il n'y a pas de calcul d oxydation (WTD=0)

MODULE stomate_wet_ch4_pt_ter_0
  ! modules used:
!  USE ioipsl
  ...
  PRIVATE
  PUBLIC ch4_wet_flux_density_0,ch4_wet_flux_density_clear_0
...
CONTAINS
  ...
  SUBROUTINE ch4_wet_flux_density_clear_0
    firstcall=.TRUE.
  END SUBROUTINE ch4_wet_flux_density_clear_0


  SUBROUTINE ch4_wet_flux_density_0 ( npts,stempdiag,tsurf,tsurf_year,veget_max,veget,&
...
!le calcul du LAI au sein du modele de Walter)
    ...
    ! Soil temperature (K)
!    REAL(r_std),DIMENSION (npts,nslm), INTENT (in)              :: tsoil

    REAL(r_std),DIMENSION (:,:), INTENT (in)              :: stempdiag ! npts, nslm
...
!! Vars initialization
    ...
    diffsa = diffair*0.66 * rpv      !! Soil air
    diffsw = (10.**(-4.)) * diffsa   !! Soil water
    diffwa = (10.**(-4.)) * diffair  !! Standing water
...
!=>  je dois modifier la valeur max d indices des boucles  n2,n3=f(ipoint)
    ...
    !CALL ftrace_region_begin("Sread")
    ...
    DO i=1,nvm
      DO ipoint = 1,npts
        IF (natural(i)) THEN ! for natural PFTs
...
!!!!tropical PFT
    ...
    pwater=0.0 !VALEUR DE LA WTD; en principe, entree du modele de Walter; ici constante
    rwater(:) = pwater
    !nwater = NINT(rwater)
    DO ipoint = 1,npts
...
!les parties suivantes sont differentes entre stomate_wet_ch4_pt_ter_0 et stomate_wet_ch4_pt_ter_m10
    ...
    DO i = 1,n3-1
      ...
      DO ipoint = 1,npts
        ...
        !par defaut ici on a toujours (nw(ipoint) .EQ. ns)
...
!!$            a(ipoint,i,j2(ipoint))=2.+rkh*(amhalf+aphalf-h**2*p)
!!$            a(ipoint,i,j3(ipoint))=-rkh*aphalf 
!!$        ELSEIF ((nw(ipoint) .LT. ns-1) .AND. (i .EQ. n1(ipoint)+1)) THEN
!!$            !i=n1+1
!!$            j1(ipoint)=n1(ipoint)
...
!!$            a(ipoint,i,j2(ipoint))=2.+rkh*(amhalf+aphalf-h**2*p)
!!$            a(ipoint,i,j3(ipoint))=-rkh*aphalf 
!!$        ELSEIF ((nw(ipoint) .LT. ns-1) .AND. (i .GE. n1(ipoint)+2) .AND. (i .LE. n2-1)) THEN            
!!$            !DO i=n1+2,n2-1
!!$            j1(ipoint)=j1(ipoint)+1
!!$            j2(ipoint)=j2(ipoint)+1
!!$            j3(ipoint)=j3(ipoint)+1
...
!!$            a(ipoint,i,j3(ipoint))=-rkh*aphalf
!!$            !ENDDO        
!!$        ELSEIF ((nw(ipoint) .LT. ns-1) .AND. (i .EQ. n2)) THEN
!!$            !i=n2
!!$            j1(ipoint)=n2-1
!!$            j2(ipoint)=n2
!!$            j3(ipoint)=n2+1
...
!!$            a(ipoint,i,j2(ipoint))=2.+rkh*(amhalf+aphalf-h**2*p)
!!$            a(ipoint,i,j3(ipoint))=-rkh*aphalf          
!!$        ELSEIF ((nw(ipoint) .LT. ns-1) .AND. (i .GE. n2+1) .AND. (i .LE. n3-1)) THEN
!!$            !DO i=n2+1,n3-1
!!$            j1(ipoint)=j1(ipoint)+1
!!$            j2(ipoint)=j2(ipoint)+1
!!$            j3(ipoint)=j3(ipoint)+1
...
!toujours vrai ici: nw(ipoint) .EQ. ns)
        ...
        DO j=2,n3
          DO ipoint =1,npts
            IF (j .LE. ndim(ipoint)) THEN
                ...
                ENDIF
!!pss
!                write (2, *) 'peng debug, n3, n1, j, ipoint, bet, gam, tria, trib, trir, triu', n3, n1(ipoint), j, ipoint, bet(ipoint), gam(ipoint, j), tria(ipoint, j), trib(ipoint, j), trir(ipoint, j), triu(ipoint, j-1)
...
END SUBROUTINE ch4_wet_flux_density_0
!********************************************************************
!!$
...
END MODULE stomate_wet_ch4_pt_ter_0
