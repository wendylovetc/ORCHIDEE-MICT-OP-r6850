...
MODULE grassland_fonctions
  ...
  INTERFACE Euler_funct
    MODULE PROCEDURE Euler_funct_scalar, Euler_funct_1d, Euler_funct_2d, Euler_funct_3d
  END INTERFACE
...
CONTAINS
  ...
  !!!!!! EULER
  !!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Euler_funct_scalar( dt, dY, Y)
    ...
    REAL(R_std), INTENT(in)        :: dY     
    ! result
    REAL(R_std), INTENT(inout)     :: Y

    Y = Y + dt*dY

  END SUBROUTINE Euler_funct_scalar

  SUBROUTINE Euler_funct_1d( dt, dY, Y)
    ! Y(n+1) = Y(n) + h*F(tn,Y(n)) 
    ! d'ou : 
    ! Y = Yp + h*dY
    ! ou h = dt
    ...
    REAL(R_std), INTENT(in)                      :: dt      
    REAL(R_std), DIMENSION(:), INTENT(in)        :: dY     
    ! result
    REAL(R_std), DIMENSION(:), INTENT(inout)     :: Y

    Y(:) = Y(:) + dt*dY(:)

  END SUBROUTINE Euler_funct_1d


  SUBROUTINE Euler_funct_2d(dt, dY, Y) ! old Euler_X
    ! matrix
    REAL(R_std), INTENT(in)      :: dt   
    REAL(R_std), DIMENSION(:, :), INTENT(in) :: dY   
    ! result
  ...
  END SUBROUTINE Euler_funct_2d

  SUBROUTINE Euler_funct_3d(dt, dY, Y) ! new Euler_X
    ! matrix
    REAL(R_std), INTENT(in)      :: dt   
    REAL(R_std), DIMENSION(:,:,:), INTENT(in) :: dY   
    ! result
