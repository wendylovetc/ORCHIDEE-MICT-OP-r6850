...
!! yidi
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:,:) :: phytomer_age         !! age of the phytomers (days)
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:,:) :: bm_phytomer          !! Each PHYTOMER mass, from sapabove
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:,:) :: bm_FFB               !! FRUIT mass for each PHYTOMER, from sapabove
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:)   :: PHYbm                !! PHYTOMER mass, from sapabove
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:)   :: FFBbm                !! Fruit mass for all phytomers
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:)     :: FFBharvest           !! harvest fruit biomass
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:)   :: last_lai             !! leaf area index
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:,:)   :: sup_flag             !! flag of using sup_LtoLSR
  REAL(r_std), ALLOCATABLE,SAVE,DIMENSION(:)     :: PHYturn              !! Turnover amount of phytomers in total
!! yidi
...
!$OMP THREADPRIVATE(wspeed_daily)
!endspit
...
!$OMP THREADPRIVATE(gpp_week)
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)    :: VPD_daily             !! xchen Daily VPD
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)    :: VPD_week              !! xchen weekly VPD
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)        :: swdown_daily           !! xchen daily swdown   
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)        :: swdown_week            !! xchen weekly swdown
...
!---
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: npp_daily            !! Daily net primary productivity per ground area 
                                                                         !! @tex $(gC m^{-2} dt_stomate^{-1})$ @endtex 
!!@chy: here no need to re-declare. these lines are commented. it's correct
!!to do this.
!!  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: lai_cl1              !! @xchen add lai of different leaf age classes 
!!  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: lai_cl2              !!  
!!  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: lai_cl3              !!  
!!  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: lai_cl4              !!  
...
!$OMP THREADPRIVATE(npp_equil)
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:,:)    :: veget_max_forcing  !! 
                                                                         !! @tex $(gC m^{-2} year^{-1})$ @endtex
!$OMP THREADPRIVATE(veget_max_forcing)
...
!$OMP THREADPRIVATE(veget_max_fm)
...
!$OMP THREADPRIVATE(tday_counter)
...
!$OMP THREADPRIVATE(wut_cm_daily)
...
!!@xchen
   !! REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)  :: lai_cl1
   !! REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)  :: lai_cl2                  
   !! REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)  :: lai_cl3                  
   !! REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)  :: lai_cl4                  
    ...
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm),   INTENT (out)  :: soilc_total        !! total soil carbon for use in thermal calcs
   ...
   IF (ok_co2 .AND. allow_forcing_write) THEN
     ...
     !! 1.6.4.2 Allocate memory for variables containing forcing data  
         ...
         IF (is_root_prc) THEN
            ...
            CALL nccheck (NF90_CREATE (TRIM(stomate_forcing_name),IOR(NF90_SHARE,NF90_NETCDF4),forcing_id))

            ! Add variable attribute
            ! Note ::iim_g and ::jjm_g are dimensions of the global field and 
            ! ::nbp_glo is the number of global continental points
            CALL nccheck (NF90_PUT_ATT (forcing_id,NF90_GLOBAL,'dt_sechiba',dt_sechiba))
            CALL nccheck (NF90_PUT_ATT (forcing_id,NF90_GLOBAL,'dt_stomate',dt_stomate))
            CALL nccheck (NF90_PUT_ATT (forcing_id,NF90_GLOBAL, &
            ...
            ! Add new dimension
            CALL nccheck (NF90_DEF_DIM (forcing_id,'points',nbp_glo,d_id(1)))
            CALL nccheck (NF90_DEF_DIM (forcing_id,'layers',nslm,d_id(2)))
            CALL nccheck (NF90_DEF_DIM (forcing_id,'pft',nvm,d_id(3)))
            direct=2
            CALL nccheck (NF90_DEF_DIM (forcing_id,'direction',direct,d_id(4)))
            nneigh=8
            CALL nccheck (NF90_DEF_DIM (forcing_id,'nneigh',nneigh,d_id(5)))
            CALL nccheck (NF90_DEF_DIM (forcing_id,'time',nsft,d_id(6)))
            CALL nccheck (NF90_DEF_DIM (forcing_id,'nbparts',nparts,d_id(7)))
            CALL nccheck (NF90_DEF_DIM (forcing_id,'ngrnd',ngrnd,d_id(8)))
            ! Add new variable
            ...
            ier = NF90_DEF_VAR (forcing_id,'lai', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid)
!!xchen
!!            ier = NF90_DEF_VAR (forcing_id,'lai_cl1', &                             
!!                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid)
!!            ier = NF90_DEF_VAR (forcing_id,'lai_cl2', &                         
!!                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid)
!!            ier = NF90_DEF_VAR (forcing_id,'lai_cl3', &                         
!!                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid)
!!            ier = NF90_DEF_VAR (forcing_id,'lai_cl4', &                         
!!                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid) 
!!            ier = NF90_ENDDEF (forcing_id)
            ...
            CALL nccheck (NF90_INQ_VARID (forcing_id,'points',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid, &
                 & (/(REAL(i,r_std),i=1,nbp_glo) /)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'layers',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,nslm)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'pft',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,nvm)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'direction',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,2)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'nneigh',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,8)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'time',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,nsft)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'nbparts',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,(/(REAL(i,r_std),i=1,nparts)/)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'index',vid))  
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,REAL(index_g,r_std)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'contfrac',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,REAL(contfrac_g,r_std)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'lalo',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,lalo_g))
            !ym attention a neighbours, a modifier plus tard      
            CALL nccheck (NF90_INQ_VARID (forcing_id,'neighbours',vid))
            CALL nccheck (NF90_PUT_VAR(forcing_id,vid,REAL(neighbours_g,r_std)))
            CALL nccheck (NF90_INQ_VARID (forcing_id,'resolution',vid))
            CALL nccheck (NF90_PUT_VAR (forcing_id,vid,resolution_g))
         ENDIF ! is_root_prc
   ...
   !! 1.4.7 write forcing file for forcesoil
      ...
      IF ( TRIM(stomate_Cforcing_name) /= 'NONE' ) THEN
         ...
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating control_moist', 'Error code=', ier)
         ALLOCATE(npp_equil(kjpindex,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating npp_equil', 'Error code=', ier)
         ALLOCATE(npp_tot(kjpindex), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating npp_tot', 'Error code=', ier)
         ALLOCATE(control_temp(kjpindex,nlevs,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating control_temp', 'Error code=', ier)
         ALLOCATE(soilcarbon_input(kjpindex,ncarb,nvm,nparan*nbyear), stat=ier) 
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating soilcarbon_input', 'Error code=', ier)
         
         ALLOCATE(veget_max_forcing(kjpindex,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating veget_max_forcing', 'Error code=', ier)
   ...
   ! 1.4.7b write forcing file for the soil permafrost module?
      ...
      IF ( TRIM(Cforcing_permafrost_name) /= 'NONE' ) THEN
         ...
         ALLOCATE(soilcarbon_input_2pfcforcing(kjpindex,ncarb,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating soilcarbon_input_2pfcforcing', 'Error code=', ier)
         ALLOCATE(pb_2pfcforcing(kjpindex,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating pb_2pfcforcing', 'Error code=', ier)
         ALLOCATE(snow_2pfcforcing(kjpindex,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating snoz_2pfcforcing', 'Error code=', ier)
         ALLOCATE(tprof_2pfcforcing(kjpindex,ndeep,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating tprof_2pfcforcing', 'Error code=', ier)
         ALLOCATE(fbact_2pfcforcing(kjpindex,ndeep,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating fbact_2pfcforcing', 'Error code=', ier)
         ALLOCATE(hslong_2pfcforcing(kjpindex,ndeep,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating hslong_2pfcforcing', 'Error code=', ier)
         ALLOCATE(veget_max_2pfcforcing(kjpindex,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating veget_max_2pfcforcing', 'Error code=', ier)
         ALLOCATE(rprof_2pfcforcing(kjpindex,nvm,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating rprof_2pfcforcing', 'Error code=', ier)
         ALLOCATE(tsurf_2pfcforcing(kjpindex,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating tsurf_2pfcforcing', 'Error code=', ier)
         !adding another two snow forcings
         ALLOCATE(snowdz_2pfcforcing(kjpindex,nsnow,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating snowdz_2pfcforcing', 'Error code=', ier)
         ALLOCATE(snowrho_2pfcforcing(kjpindex,nsnow,nparan*nbyear), stat=ier)
         IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating snowrho_2pfcforcing', 'Error code=', ier)
...
!!ENDNVADD
   ...
   IF (CH4_calcul) CALL stomate_wet_ch4_initialize(kjpindex, rest_id_stom, itime)
...
SUBROUTINE stomate_main &
       ...
       &  lai, &
!!@xchen lai_cl1-4 of leaf age class 1-4 for claculating the PC in Wu's paper
       &  lai_cl1, lai_cl2, lai_cl3, lai_cl4, laitop, laidow,VPD, ave_VPD, &
!!@xchen                                                                        
       &  vcmax_cl1, vcmax_cl2,vcmax_cl3,vcmax_cl4, &   
       &  frac_age, height, veget, veget_max, &
       &  veget_max_new, &
       &  vegetnew_firstday,totfrac_nobio_new, fraclut, &
...
!!!! crops xuhui
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl1          !!@xchen lai of leaf class age 1-4 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl2          !! 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl3          !! 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl4          !!
!! xchen add vpd for gpp simulation in Amazon                                   
    REAL(r_std), DIMENSION(kjpindex),     INTENT (in)       :: VPD              !! Vapor Pressure Deficit (kPa)
    REAL(r_std), DIMENSION(kjpindex),     INTENT (out)       :: ave_VPD              !! Vapor Pressure Deficit (kPa)
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: laitop           !!xchen Wu (science, 2016) sunlit leaf area index @tex ($m^2 m^{-2}$) 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: laidow            !!xchen Wu (science 2016) shade leaf area index @tex ($m^2 m^{-2}$) 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: vcmax_cl1          !!@xchen vcmax of leaf class age 1-4 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: vcmax_cl2          !!     
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: vcmax_cl3          !!     
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: vcmax_cl4          !!   
 
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)  :: veget_max_new     !! New "maximal" coverage fraction of a PFT (LAI ->  
                                                                         !! infinity) on ground only if LastTsYear is
                                                                         !! activated (unitless)
    ...
    REAL(r_std),DIMENSION(kjpindex,12),INTENT(inout)  :: harvest_matrix  !! New "maximal" coverage fraction of a PFT (LAI -> 
                                                                         !! infinity) on ground only if LastTsYear is 
    REAL(r_std),DIMENSION(kjpindex,12),INTENT(in)  :: harvest_biomass  !! New "maximal" coverage fraction of a PFT (LAI -> 
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)  :: bound_spa      !! New "maximal" coverage fraction of a PFT (LAI -> 
                                                                         !! infinity) on ground only if LastTsYear is 
    REAL(r_std),DIMENSION(kjpindex,nvmap),INTENT(in)  :: newvegfrac      !! New "maximal" coverage fraction of a PFT (LAI -> 
                                                                         !! infinity) on ground only if EndOfYear is 
    ...
    LOGICAL, INTENT(in)                             :: LastTsYear       !! Flag set to true for the first sechiba time step on the year.
...
!end gmjc
    !! 0.2 Output variables
    REAL(r_std),DIMENSION(kjpindex,nvm)  :: delta_turnover          !!xchen1111
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: gpp_d                    !! Gross primary productivity per ground area 
                                                                              !! @tex $(??gC m^{-2} dt_stomate^{-1})$ @endtex 
!!xchen
    REAL(r_std), DIMENSION(kjpindex)              :: VPD_d              !! Vapor Pressure Deficit (kPa)
...
!_ ================================================================================================================================
    ...
    IF (printlev >= 4) THEN
       WRITE(numout,*) 'stomate_main: days_since_beg=',days_since_beg,' ymds=', year_end, month_end, day_end, sec_end, &
    ...
    ! chaoyue: because stomate_main is excuted every sechiba time step,
    ! but we need to do the following adjustment only once. Therefore we
    ! limit these operations under the condition of FirstTsYear. Take this
    ! feature into account when modifying this part of code.
    IF (FirstTsYear) THEN
    ...
    ENDDO
!!xchen
    VPD_d(:) = VPD(:)/10* one_day/dt_sechiba

    !permafrost:  get the residence time for soil carbon
    IF ( printlev>=3 ) WRITE(*,*) 'cdk debug stomate: prep to calc fbact'
    fbact = microactem(tdeep, frozen_respiration_func, hsdeep, kjpindex, ndeep, nvm, zz_deep)
...
!    ENDDO ! finish the calculation of cuh in each time step of sechiba's loop
    ...
    IF (ANY(ok_LAIdev)) THEN
        ...
        CALL stomate_accu(kjpindex, do_slow, uphoi, phoi)  ! finish of the calculation of photoperiod, phoi, in hours
        ! calculation of the daily accumulated potential evapotranspiration
        CALL stomate_accu(kjpindex, do_slow, evapot_corr, evapot_daily)
    ENDIF
    !!!!! end for crops, xuhui
    ...
    CALL stomate_accu (kjpindex, do_slow, litterhumdiag, litterhum_daily)
    CALL stomate_accu (kjpindex, do_slow, t2m,           t2m_daily)
    CALL stomate_accu (kjpindex, do_slow, temp_sol,      tsurf_daily)
    CALL stomate_accu (kjpindex, nslm, do_slow, stempdiag,     tsoil_daily)
    CALL stomate_accu (kjpindex, nslm, do_slow, shumdiag,      soilhum_daily)
    CALL stomate_accu (kjpindex, do_slow, precip,        precip_daily)
    CALL stomate_accu (kjpindex, nvm, do_slow, gpp_d,         gpp_daily)
  !!xchen
    CALL stomate_accu (kjpindex,  do_slow, VPD_d,         VPD_daily)
    CALL stomate_accu (kjpindex, do_slow, swdown,         swdown_daily)

!gmjc # trunk do not introduce snow in stomate
    CALL stomate_accu (kjpindex, do_slow, precip_snow, snowfall_daily)
    CALL stomate_accu (kjpindex, do_slow, snow,        snowmass_daily)
    CALL stomate_accu (kjpindex, do_slow, tmc_topgrass, tmc_topgrass_daily)
!end gmjc

    !ditto for permafrost variables
    IF ( do_daily_permafrost ) THEN
       CALL stomate_accu(kjpindex, ndeep, nvm, do_slow, tdeep, tdeep_daily)
       CALL stomate_accu(kjpindex, ndeep, nvm, do_slow, hsdeep, hsdeep_daily)
       !           CALL stomate_accu( do_slow, fbact(:,:,iv), fbact_daily(:,:,iv))
       CALL stomate_accu(kjpindex, ndeep, nvm, do_slow, prmfrst_soilc_tempctrl,prmfrst_soilc_tempctrl_daily)
       CALL stomate_accu(kjpindex, do_slow, snow, snow_daily)
       CALL stomate_accu(kjpindex, do_slow, pb * 100., pb_pa_daily)
       CALL stomate_accu(kjpindex, do_slow, temp_sol, temp_sol_daily)
       !adding two forcings
       CALL stomate_accu(kjpindex, nsnow, do_slow, snowdz, snowdz_daily)
       CALL stomate_accu(kjpindex, nsnow, do_slow, snowrho, snowrho_daily)
    ENDIF
    ...
    !spitfire
    CALL stomate_accu (kjpindex, do_slow, wspeed, wspeed_daily)
!    t2m_max_daily(:) = MAX( t2m(:), t2m_max_daily(:) )
    !endspit
...
!    WRITE(numout,*) 'lai before stomate_resp:',lai(1,12:14)
    CALL maint_respiration &
         & (kjpindex,lai,t2m,t2m_longterm,stempdiag,height, &
...
!end gmjc
    ...
    IF ( ok_pc ) THEN
       ...
       ELSE
          ...
          CALL stomate_accu(kjpindex, ncarb, nvm, do_slow, soilcarbon_input_inst, soilcarbon_input_daily)
...
!!!!! crops
   IF (ANY(ok_LAidev)) THEN
      ...
      CALL stomate_accu(kjpindex, nvm, 3, do_slow, stempdiag_cm, wut_cm_daily)

      ! calculate daily mean values for soil moisture for root zone
      CALL stomate_accu(kjpindex, nvm, 3, do_slow, shumdiag_cm, wus_cm_daily)
   ENDIF
!!!!! xuhui
...
!! 5. Daily processes - performed at the end of the day
    ...
    IF (do_slow) THEN
       WRITE(numout,*) "Number of days: ", days_since_beg
       ...
       IF (CH4_calcul) THEN
         CALL stomate_wet_ch4_main(kjpindex, stempdiag, tsurf_daily, tsurf_year, &
...
!!xchen
          lai_cl1(:,ibare_sechiba) = zero  
          lai_cl2(:,ibare_sechiba) = zero                                           
          lai_cl3(:,ibare_sechiba) = zero                                           
          lai_cl4(:,ibare_sechiba) = zero                                          
          laitop(:,ibare_sechiba) = zero                                        
          laidow(:,ibare_sechiba) = zero    
...
!          WRITE (numout,*) 'LAI before alloc: ',lai(:,12:14)
       ...
       CALL season &
            ...
            &           VPD_daily, VPD_week, swdown_daily, swdown_week, & !!xchen
            &           maxhumrel_lastyear, maxhumrel_thisyear, &
            &           minhumrel_lastyear, minhumrel_thisyear, &
            &           maxgppweek_lastyear, maxgppweek_thisyear, &
            &           gdd0_lastyear, gdd0_thisyear, &
...
!              WRITE(numout,*) 'in_cycle(:,11): ',in_cycle(:,11)
          ...
          CALL StomateLpj &
               ...
               &             read_cf_fine,cf_fine,read_cf_coarse,cf_coarse,read_ratio_flag,ratio_flag,read_ratio,ratio, days_since_beg, &
               !endspit
...
!gmjc
               ...
               &             LastTsYear, &          !! Flag set to true for the first sechiba
!gmjc top 5 layer grassland soil moisture for grazing
...
!                 IF ( SUM(SUM(matrix_prc_rot(i,:,:))) .GT. min_stomate) THEN
              ...
              ENDDO
          ENDIF
          IF (LastTsYear) THEN  ! update crop calendar
              tday_counter = 0
              !!! update plantdate_cyc
              DO j = 2,nvm
                IF (ok_LAIdev(j)) THEN
                    DO i = 1,kjpindex
                        IF (plantdate_now(i,j) .GT. year_length_in_days) THEN 
                        !!! considering the next year planting date
                            plantdate_now(i,j) = plantdate_now(i,j) - year_length_in_days
                        ENDIF
                    ENDDO
                ENDIF
              ENDDO
              IF (printlev>=3 .AND. ok_rotate) THEN
                WRITE(numout,*) 'after LastTsYear:'
          ...
          IF (TRIM(stomate_Cforcing_name) /= 'NONE') THEN
             ...
             iatt = stomate_io_tstep_index(day_of_year, year0, nbyear, nparan)
             ...
             veget_max_forcing(:,:,iatt) = veget_max_forcing(:,:,iatt) + veget_max(:,:)
          ENDIF
...
9010  FORMAT(A52,F17.14)
       ...
       IF (spinup_analytic) THEN
          nbp_accu(:) = nbp_accu(:) + (SUM(nep_daily(:,:) * veget_max(:,:),dim=2) - SUM(convflux(:,:) + cflux_prod10(:,:) + &
                    cflux_prod100(:,:),DIM=2)  - harvest_above(:))/1e3 
       ENDIF

       !! 5.11 Reset daily variables
       !!      Include for all variables used in stomate_accu
       humrel_daily(:,:) = zero
       ...
       precip_daily(:) = zero
       gpp_daily(:,:) = zero
       VPD_daily(:) = zero !!xchen
       swdown_daily(:) = zero !!xchen                                           
  ...
  !! 6. Outputs from Stomate
    ...
    co2_flux(:,:) = resp_hetero(:,:) + resp_maint(:,:) + resp_growth(:,:) &
         & + (co2_fire(:,:)-co2_to_bm_dgvm(:,:))*veget_cov_max(:,:)*dt_sechiba/one_day &
         & - gpp(:,:)
    ...
    IF (spinup_analytic) THEN
       ...
       IF (LastTsYear) THEN
          ...
          IF( MOD(global_years, spinup_period) == 0 ) THEN
             ...
             nbp_flux(:) = nbp_accu(:) / ( spinup_period * year_length_in_days)
             ! Reset the values
             nbp_accu(:) = zero
...
!_ ================================================================================================================================
  ...
  SUBROUTINE stomate_finalize (kjit, kjpindex, index, clay, &
    ...
    REAL(r_std), DIMENSION(kjpindex),   INTENT (inout)            :: depth_organic_soil !! how deep is the organic soil?
!!@xchen                                                                        
!!    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl1                 
 !!   REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl2                 
 !!   REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl3                 
 !!   REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: lai_cl4   
    ...
    REAL(r_std),ALLOCATABLE,DIMENSION(:,:)        :: npp_equil_g              !! Equilibrium NPP written to forcesoil 
                                                                              !! @tex $(gC m^{-2} year^{-1})$ @endtex, parallel 
                                                                              !! computing 
    REAL(r_std),ALLOCATABLE,DIMENSION(:,:,:)      :: veget_max_forcing_g      !! Veget_max for forcesoil 
...
!! yidi
        ...
        &          MatrixV, VectorU, previous_stock, current_stock, assim_param, &
        &          deepC_a, deepC_s, deepC_p, O2_soil, CH4_soil, O2_snow,CH4_snow, &
        &          thawed_humidity, depth_organic_soil, altmax,fixed_cryoturbation_depth, & 
        &         tsurf_year, &!) !pss- +:wetlabd CH4 emissions
    !gmjc
        &         wshtotsum, sr_ugb, sla_calc, nb_ani, grazed_frac, &
        &         import_yield, t2m_14, litter_not_avail, nb_grazingdays, &
        &         after_snow, after_wet, wet1day, wet2day)
...
! end crop variables, xuhui
    ...
    IF (ok_co2 .AND. allow_forcing_write ) THEN
      IF ( TRIM(stomate_forcing_name) /= 'NONE' ) THEN  
         ...
         ! Close forcing file
         IF (is_root_prc) CALL nccheck( NF90_CLOSE (forcing_id) )
         forcing_id=-1
      END IF
    END IF
    ...
    IF (TRIM(stomate_Cforcing_name) /= 'NONE' ) THEN 
      ...
      DO iatt = 1, nparan*nbyear
         IF ( nforce(iatt) > 0 ) THEN
            ...
            veget_max_forcing(:,:,iatt) = &
                 & veget_max_forcing(:,:,iatt)/REAL(nforce(iatt),r_std)
         ELSE
            WRITE(numout,*) &
      ...
      IF (is_root_prc) THEN
         ...
         ALLOCATE(veget_max_forcing_g(nbp_glo,nvm,nparan*nbyear))
      ENDIF
      ...
      CALL gather(veget_max_forcing,veget_max_forcing_g)
      ...
      IF (is_root_prc) THEN
         ...
         ! Create new netCDF dataset
         ier = NF90_CREATE (TRIM(stomate_Cforcing_name),NF90_NETCDF4 ,Cforcing_id)
         ...
         CALL nccheck(NF90_PUT_ATT (Cforcing_id,NF90_GLOBAL, &
              &                        'kjpindex',REAL(nbp_glo,r_std)))
         CALL nccheck(NF90_PUT_ATT (Cforcing_id,NF90_GLOBAL, &
         ...
         CALL nccheck(NF90_PUT_ATT (Cforcing_id,NF90_GLOBAL, &
              &                        'dt_sechiba',REAL(dt_sechiba,r_std)))
         
         ! Add new dimension
         CALL nccheck(NF90_DEF_DIM (Cforcing_id,'points',nbp_glo,d_id(1)))
         CALL nccheck(NF90_DEF_DIM (Cforcing_id,'carbtype',ncarb,d_id(2)))
         CALL nccheck(NF90_DEF_DIM (Cforcing_id,'vegtype',nvm,d_id(3)))
         CALL nccheck(NF90_DEF_DIM (Cforcing_id,'level',nlevs,d_id(4)))
         CALL nccheck(NF90_DEF_DIM (Cforcing_id,'time_step',nparan*nbyear,d_id(5)))
         
         ! Add new variable
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'points',    r_typ,d_id(1),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'carbtype',  r_typ,d_id(2),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'vegtype',   r_typ,d_id(3),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'level',     r_typ,d_id(4),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'time_step', r_typ,d_id(5),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'index',     r_typ,d_id(1),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'clay',      r_typ,d_id(1),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'soilcarbon_input',r_typ, &
              &                        (/ d_id(1),d_id(2),d_id(3),d_id(5) /),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'control_moist',r_typ, &
              &                        (/ d_id(1),d_id(4),d_id(5) /),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'control_temp',r_typ, &
              &                        (/ d_id(1),d_id(4),d_id(5) /),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'npp_equil',r_typ, &
              &                        (/ d_id(1),d_id(5) /),vid))
         CALL nccheck(NF90_DEF_VAR (Cforcing_id,'veget_max',r_typ, &
              &                        (/ d_id(1),d_id(3),d_id(5) /),vid))
         CALL nccheck(NF90_ENDDEF (Cforcing_id))
         
         ! Given the name of a varaible, nf90_inq_varid finds the variable 
         ! ID (::vid). Put data value(s) into variable ::vid 
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'points',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, &
              &                          (/(REAL(i,r_std),i=1,nbp_glo)/)))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'carbtype',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, &
              &                        (/(REAL(i,r_std),i=1,ncarb)/)))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'vegtype',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, &
              &                            (/(REAL(i,r_std),i=1,nvm)/)))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'level',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, &
              &                          (/(REAL(i,r_std),i=1,nlevs)/)))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'time_step',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, &
              &                          (/(REAL(i,r_std),i=1,nparan*nbyear)/)))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'index',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, REAL(index_g,r_std) ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'clay',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, clay_g ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'soilcarbon_input',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, soilcarbon_input_g ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'control_moist',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, control_moist_g ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'control_temp',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, control_temp_g ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'npp_equil',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, npp_equil_g ))
         CALL nccheck(NF90_INQ_VARID (Cforcing_id,'veget_max',vid))
         CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, veget_max_forcing_g ))
      ...
      IF (is_root_prc) THEN
         ...
         DEALLOCATE(veget_max_forcing_g)
      ENDIF
    ...
    ENDIF
    IF (CH4_calcul) CALL stomate_wet_ch4_finalize(rest_id_stomate, itime)
...
!_ ================================================================================================================================
  ...
  !! 4. Allocate memory for STOMATE's variables
    ...
    ENDIF
!!xchen
    ALLOCATE(VPD_daily(kjpindex),stat=ier)                                   
    l_error = l_error .OR. (ier /= 0)                                           
    IF (l_error) THEN                                                           
       WRITE(numout,*) 'Memory allocation error for VPD_daily. We stop. We need kjpindex words',kjpindex,nvm
       STOP 'stomate_init'                                                      
    ENDIF
    ALLOCATE(VPD_week(kjpindex),stat=ier)                                      
    l_error = l_error .OR. (ier /= 0)                                           
    IF (l_error) THEN                                                           
       WRITE(numout,*) 'Memory allocation error for VPD_week. We stop. We need kjpindex words',kjpindex,nvm
       STOP 'stomate_init'                                                      
    ENDIF 
    ALLOCATE(swdown_daily(kjpindex),stat=ier)                                   
    l_error = l_error .OR. (ier /= 0)                                           
    IF (l_error) THEN                                                           
       WRITE(numout,*) 'Memory allocation error for swdown_daily. We stop. We need kjpindex words',kjpindex,nvm
       STOP 'stomate_init'                                                      
    ENDIF   
    ALLOCATE(swdown_week(kjpindex),stat=ier)                                    
    l_error = l_error .OR. (ier /= 0)                                           
    IF (l_error) THEN                                                           
       WRITE(numout,*) 'Memory allocation error for swdown_week. We stop. We need kjpindex words',kjpindex,nvm
       STOP 'stomate_init'                                                      
    ENDIF   
...
!!@xchen
!!    ALLOCATE(lai_cl1(kjpindex,nvm),stat=ier)                                  
!!    l_error = l_error .OR. (ier /= 0)                                           
!!    IF (l_error) THEN                                                           
!!       WRITE(numout,*) 'Memory allocation error for lai_cl1. We stop. We need kjpindex*nvm words',kjpindex,nvm
!!      STOP 'stomate_init'                                                      
!!    ENDIF 
!!    ALLOCATE(lai_cl2(kjpindex,nvm),stat=ier)                                    
!!    l_error = l_error .OR. (ier /= 0)                                           
!!    IF (l_error) THEN                                                           
!!       WRITE(numout,*) 'Memory allocation error for lai_cl2. We stop. We need kjpindex*nvm words',kjpindex,nvm
!!       STOP 'stomate_init'                                                      
!!    ENDIF 
!!    ALLOCATE(lai_cl3(kjpindex,nvm),stat=ier)                                    
!!    l_error = l_error .OR. (ier /= 0)                                           
!!    IF (l_error) THEN                                                           
!!       WRITE(numout,*) 'Memory allocation error for lai_cl3. We stop. We need kjpindex*nvm words',kjpindex,nvm
!!       STOP 'stomate_init'                                                      
!!    ENDIF 
!!    ALLOCATE(lai_cl4(kjpindex,nvm),stat=ier)                                    
!!    l_error = l_error .OR. (ier /= 0)                                           
!!    IF (l_error) THEN                                                           
!!       WRITE(numout,*) 'Memory allocation error for lai_cl4. We stop. We need kjpindex*nvm words',kjpindex,nvm
!!       STOP 'stomate_init'                                                      
!!    ENDIF 
...
!! yidi
    IF ( ANY(is_oilpalm)) THEN
       ALLOCATE(phytomer_age(kjpindex,nvm,nphs),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for phytomer_age. We stop. We need kjpindex*nvm*nphs words', & 
          &      kjpindex,nvm,nphs
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(bm_phytomer(kjpindex,nvm,nphs),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for bm_phytomer. We stop. We need kjpindex*nvm*nphs words', & 
          &      kjpindex,nvm,nphs
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(bm_FFB(kjpindex,nvm,nphs),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for bm_FFB. We stop. We need kjpindex*nvm*nphs words', & 
          &      kjpindex,nvm,nphs
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(FFBbm(kjpindex,nvm),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for FFBbm. We stop. We need kjpindex*nvm words', & 
          &      kjpindex,nvm
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(PHYbm(kjpindex,nvm),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for PHYbm. We stop. We need kjpindex*nvm words', & 
          &      kjpindex,nvm
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(FFBharvest(kjpindex),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for FFBharvest. We stop. We need kjpindex words', & 
          &      kjpindex
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(PHYturn(kjpindex),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for PHYturn. We stop. We need kjpindex words', & 
          &      kjpindex
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(last_lai(kjpindex,nvm),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for last_lai. We stop. We need kjpindex words', & 
          &      kjpindex,nvm
          STOP 'stomate_init'
       ENDIF

       ALLOCATE(sup_flag(kjpindex,nvm),stat=ier)
       l_error = l_error .OR. (ier /= 0)
       IF (l_error) THEN
          WRITE(numout,*) 'Memory allocation error for sup_flag. We stop. We need kjpindex words', & 
          &      kjpindex,nvm
          STOP 'stomate_init'
       ENDIF
...
!!!!! crops
    ...
    ! chaoyue: Jinfeng suggests that N_limfert is also used by GM so it should
    ! be allocated with correct shape whatever the value of ok_LAIdev.
...
!        l_error = l_error .OR. (ier /= 0)
    ...
    IF (l_error) THEN
       STOP 'stomate_init: error in memory allocation beforefor somsenreste'
    ENDIF 
    WRITE(numout,*) 'stomate_init: before box ulai'

    ! only allocated under specific cases    
    IF ( any(ok_LAIdev(:)) .AND. any(SP_codlainet(:) .eq. 2) ) THEN
        ALLOCATE(histgrowthset(kjpindex,nvm,300,5),stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating histgrowthset', 'Error code=', ier)
        ALLOCATE(hist_sencourset(kjpindex,nvm),stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating hist_sencourset', 'Error code=', ier)
        ALLOCATE(hist_latestset(kjpindex,nvm),stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating hist_latestset', 'Error code=', ier)
        ALLOCATE(doyhiststset(kjpindex,nvm),stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating doyhiststset', 'Error code=', ier)
    ELSE
        ! ugly code, it requires an overall clean (crop vs stomate) 
        ! an argument must be allocated when passed
        ALLOCATE(histgrowthset(1,1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating histgrowth', 'Error code=', ier)
        ALLOCATE(hist_sencourset(1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating hist_sensourset', 'Error code=', ier)
        ALLOCATE(hist_latestset(1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating hist_latestset', 'Error code=', ier)
        ALLOCATE(doyhiststset(1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating doyhiststset', 'Error code=', ier)
    ENDIF
    ...
    IF ( ANY(ok_LAIdev(:)) .AND. ANY(SP_codlainet(:) .eq. 3) ) THEN
        ...
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_ulai', 'Error code=', ier)
        ALLOCATE(box_ndays(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_ndays', 'Error code=', ier)
        ALLOCATE(box_lai(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_lai', 'Error code=', ier)
        ALLOCATE(box_lairem(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_lairem', 'Error code=', ier)
        ALLOCATE(box_tdev(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_tdev', 'Error code=', ier)
        ALLOCATE(box_biom(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_biom', 'Error code=', ier)
        ALLOCATE(box_biomrem(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_biomrem', 'Error code=', ier)
        ALLOCATE(box_durage(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_durage', 'Error code=', ier)
        ALLOCATE(box_somsenbase(kjpindex, nvm, nboxmax), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_somsenbase', 'Error code=', ier)
        write(*,*) 'xuhui: box module allocated'
    ELSE
        ALLOCATE(box_ulai(1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_ulai', 'Error code=', ier)
        ALLOCATE(box_ndays(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_ndays', 'Error code=', ier)
        ALLOCATE(box_lai(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_lai', 'Error code=', ier)
        ALLOCATE(box_lairem(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_lairem', 'Error code=', ier)
        ALLOCATE(box_tdev(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_tdev', 'Error code=', ier)
        ALLOCATE(box_biom(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_biom', 'Error code=', ier)
        ALLOCATE(box_biomrem(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_biomrem', 'Error code=', ier)
        ALLOCATE(box_durage(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_durage', 'Error code=', ier)
        ALLOCATE(box_somsenbase(1,1,1), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'stomate_init', 'Problem allocating box_somsenbase', 'Error code=', ier)
...
!! yidi
    ALLOCATE(horiphytomer_index(kjpindex*nphs),stat=ier)
    IF (ier /= 0) THEN
       WRITE(numout,*) 'Memory allocation error for horiphytomer_index. We stop. We need kjpindex*nphs words',kjpindex*nphs
       STOP 'stomate_init'
    ENDIF
!! yidi
...
!! yidi
    DO j = 1, nphs
       DO ji = 1, kjpindex
          horiphytomer_index((j-1)*kjpindex+ji) = index_g(ji)+(j-1)*kjpij  + offset_omp - offset_mpi
       ENDDO
    ENDDO
!! yidi
    hori_index(:) = index_g(:)
...
!!@xchen
    IF (ALLOCATED(VPD_daily)) DEALLOCATE(VPD_daily)                             
    IF (ALLOCATED(VPD_week)) DEALLOCATE(VPD_week)                           
    IF (ALLOCATED(swdown_daily)) DEALLOCATE(swdown_daily)                        
    IF (ALLOCATED(swdown_week)) DEALLOCATE(swdown_week)                         
 

    !!IF (ALLOCATED(lai_cl1)) DEALLOCATE(lai_cl1)                             
    !!IF (ALLOCATED(lai_cl2)) DEALLOCATE(lai_cl2)                                 
    !!IF (ALLOCATED(lai_cl3)) DEALLOCATE(lai_cl3)                                 
    !!IF (ALLOCATED(lai_cl4)) DEALLOCATE(lai_cl4)                                 
...
!! yidi
    IF (ALLOCATED(phytomer_age)) DEALLOCATE(phytomer_age)
    IF (ALLOCATED(bm_phytomer)) DEALLOCATE(bm_phytomer)
    IF (ALLOCATED(bm_FFB)) DEALLOCATE(bm_FFB)
    IF (ALLOCATED(PHYbm)) DEALLOCATE(PHYbm)
    IF (ALLOCATED(FFBbm)) DEALLOCATE(FFBbm)
    IF (ALLOCATED(FFBharvest)) DEALLOCATE(FFBharvest)
    IF (ALLOCATED(last_lai)) DEALLOCATE(last_lai)
    IF (ALLOCATED(sup_flag)) DEALLOCATE(sup_flag)
    IF (ALLOCATED(PHYturn)) DEALLOCATE(PHYturn)
!! yidi
...
!end gmjc
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)        :: lai             !! Leaf area index 
                                                                             !! @tex $(m^2 m{-2})$ @endtex
!@chy: here what you did is correct. no need to use lai_cl1 here.
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)        :: lai_cl1         !!@xchen lai of leaf class age 1-4 
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)        :: lai_cl2         !! 
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)        :: lai_cl3         !! 
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)        :: lai_cl4         !! 
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)           :: vcmax_cl1
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)           :: vcmax_cl2       
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)           :: vcmax_cl3       
    !!REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)           :: vcmax_cl4       
    ...
    REAL(r_std),DIMENSION(kjpindex), INTENT (out)         :: deadleaf_cover  !! Fraction of soil covered by dead leaves 
                                                                             !! (unitless) 
!! yidi
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs), INTENT(in) :: phytomer_age    !! age of the phytomers (days)
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs),INTENT(in)  :: bm_phytomer     !! Each PHYTOMER mass, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs),INTENT(in)  :: bm_FFB          !! FRUIT mass for each PHYTOMER, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm),INTENT(in)       :: PHYbm           !! PHYTOMER mass, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm),INTENT(in)       :: FFBbm           !! Fruit mass for all phytomers
!! yidi
...
!end gmjc
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm)                   :: vcmax           !! Dummy vcmax 
                                                                             !! @tex $(\mu mol m^{-2} s^{-1})$ @endtex
    REAL(r_std),DIMENSION(kjpindex,nvm)                   :: vcmax_cl1           !! Dummy vcmax 
    REAL(r_std),DIMENSION(kjpindex,nvm)                   :: vcmax_cl2           !! Dummy vcmax 
    REAL(r_std),DIMENSION(kjpindex,nvm)                   :: vcmax_cl3           !! Dummy vcmax 
    REAL(r_std),DIMENSION(kjpindex,nvm)                   :: vcmax_cl4           !! Dummy vcmax
    REAL(r_std),DIMENSION(kjpindex)                       :: ave_VPD              !! Vapor Pressure Deficit (kPa) 
...
!! yidi
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs)        :: phytomer_age_tmp     !! age of the phytomers (days)
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs)        :: bm_phytomer_tmp      !! Each PHYTOMER mass, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm,nphs)        :: bm_FFB_tmp           !! FRUIT mass for each PHYTOMER, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm)             :: PHYbm_tmp            !! PHYTOMER mass, from sapabove
    REAL(r_std), DIMENSION(kjpindex,nvm)             :: FFBbm_tmp            !! Fruit mass for all phytomers
!! yidi
...
!! yidi
       IF (ok_oilpalm) THEN
          phytomer_age_tmp(:,:,:)=phytomer_age(:,:,:)
          bm_phytomer_tmp(:,:,:)=bm_phytomer(:,:,:)
          bm_FFB_tmp(:,:,:)=bm_FFB(:,:,:)
          FFBbm_tmp(:,:)=FFBbm(:,:)
          PHYbm_tmp(:,:)=PHYbm(:,:)
       ENDIF
!! yidi
       ...
       CALL vmax (kjpindex, dt_0, leaf_age_tmp, leaf_frac_tmp,phytomer_age_tmp, bm_phytomer_tmp, bm_FFB_tmp, PHYbm_tmp, FFBbm_tmp, biomass, & !! yidi
        vcmax, vcmax_cl1,vcmax_cl2,vcmax_cl3,vcmax_cl4, VPD_week, ave_VPD, N_limfert) !!xchen
...
!!
!! RECENT CHANGE(Stomate_accu : None
!!
...
!! \n
!_ ================================================================================================================================
  SUBROUTINE stomate_accu_r1d (fidim, ldmean, field_in, field_out)
    
  !! 0. Variable and parameter declaration

    !! 0.1 Input variables
    LOGICAL,INTENT(in)                           :: ldmean    !! Flag to calculate the mean over
    INTEGER(i_std), INTENT(in)                   :: fidim     !! Array Dimensions
    REAL(r_std),DIMENSION(1:fidim),INTENT(in)    :: field_in  !! Field that needs to be accumulated
    
    !! 0.2 Modified variables
    REAL(r_std),DIMENSION(1:fidim),INTENT(inout) :: field_out !! Accumulated or mean field

!_ ================================================================================================================================

  !! 1. Accumulate field
    !DIR$ VECTOR ALIGNED
    field_out(1:fidim) = field_out(1:fidim)+field_in(1:fidim)*dt_sechiba
   
  !! 2. Mean fields

    IF (ldmean) THEN
       !DIR$ VECTOR ALIGNED
       field_out(1:fidim) = field_out(1:fidim)/dt_stomate
    ENDIF

  END SUBROUTINE stomate_accu_r1d
  
  SUBROUTINE stomate_accu_r2d (fidim, sedim, ldmean, field_in, field_out)
    
  !! 0. Variable and parameter declaration

    !! 0.1 Input variables
    LOGICAL,INTENT(in)                               :: ldmean    !! Flag to calculate the mean over
    INTEGER(i_std), INTENT(in)                       :: fidim, sedim !! Array Dimensions
    REAL(r_std),DIMENSION(fidim,sedim),INTENT(in)    :: field_in  !! Field that needs to be accumulated
    
    !! 0.2 Modified variables
    REAL(r_std),DIMENSION(fidim,sedim),INTENT(inout) :: field_out !! Accumulated or mean field

!_ ================================================================================================================================

  !! 1. Accumulate field
    !DIR$ VECTOR ALIGNED
    field_out(1:fidim,1:sedim) = field_out(1:fidim,1:sedim)+field_in(1:fidim,1:sedim)*dt_sechiba
   
  !! 2. Mean fields

    IF (ldmean) THEN
       !DIR$ VECTOR ALIGNED
       field_out(1:fidim,1:sedim) = field_out(1:fidim,1:sedim)/dt_stomate
    ENDIF

  END SUBROUTINE stomate_accu_r2d

  SUBROUTINE stomate_accu_r3d (fidim, sedim, thdim, ldmean, field_in, field_out)
    
  !! 0. Variable and parameter declaration

    !! 0.1 Input variables
    LOGICAL,INTENT(in)                                     :: ldmean    !! Flag to calculate the mean over
    INTEGER(i_std), INTENT(in)                             :: fidim, sedim, thdim !! Array Dimensions
    REAL(r_std),DIMENSION(fidim,sedim,thdim),INTENT(in)    :: field_in  !! Field that needs to be accumulated
    
    !! 0.2 Modified variables
    REAL(r_std),DIMENSION(fidim,sedim,thdim),INTENT(inout) :: field_out !! Accumulated or mean field

!_ ================================================================================================================================

  !! 1. Accumulate field
    !DIR$ VECTOR ALIGNED
    field_out(1:fidim,1:sedim,1:thdim) = field_out(1:fidim,1:sedim,1:thdim)+field_in(1:fidim,1:sedim,1:thdim)*dt_sechiba
   
  !! 2. Mean fields

    IF (ldmean) THEN
       !DIR$ VECTOR ALIGNED
       field_out(1:fidim,1:sedim,1:thdim) = field_out(1:fidim,1:sedim,1:thdim)/dt_stomate
    ENDIF
