  ...
  !Cette routine calcule les densites de flux de CH4 pour un wetlands dont la water table depth est constante dans le temps et situee à pwater_wt cm EN-DESSOUS de la surface du sol
  ...
  !Dans cette routine, il y a un calcul d oxydation (sur la partie 0;pwater_wt)
 
MODULE stomate_wet_ch4_pt_ter_wet
  ! modules used:
!  USE ioipsl
  USE pft_parameters
  ...
  PRIVATE
  PUBLIC ch4_wet_flux_density_wet,ch4_wet_flux_density_clear_wet
...
CONTAINS
  ...
  SUBROUTINE ch4_wet_flux_density_clear_wet
    firstcall=.TRUE.
  END SUBROUTINE ch4_wet_flux_density_clear_wet


  SUBROUTINE ch4_wet_flux_density_wet ( npts,stempdiag,tsurf,tsurf_year,veget_max,veget,&
     & carbon_surf,lai,uo,uold2,ch4_flux_density_tot, ch4_flux_density_dif, & 
     & ch4_flux_density_bub, ch4_flux_density_pla, catm, pwater_wet)  
...
!le travail de coherence entre les 2 modoles a ete fait pour le LAI (j ai enlevé
...
!    REAL(r_std),DIMENSION (npts,nslm), INTENT (in)              :: tsoil
    ...
    REAL(r_std),DIMENSION (npts,nslm), INTENT (in)              :: stempdiag
    ...
    ! modified by Shushi Peng for active carbon in the top 2 m soil
    !pss+++++
...
!end modif bruno
    ...
    REAL(r_std), INTENT(in)                   :: pwater_wet
...
!!!pssdebug +
!          write(2, *) 'pssdebug, nw(ipoint), ipoint, n1(ipoint), n2, diffdo(ipoint), diffup(ipoint), diffsw, diffsa', nw(ipoint), ipoint, n1(ipoint), n2, diffdo(ipoint), diffup(ipoint), diffsw, diffsa 
!!!pssdebug-
          rvmax(ipoint)=xvmax
      ENDIF
...
END SUBROUTINE ch4_wet_flux_density_wet
!********************************************************************
!!$
...
END MODULE stomate_wet_ch4_pt_ter_wet
