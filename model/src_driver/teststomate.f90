...
!gmjc
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: tmc_topgrass
!end gmjc
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: humcste_use
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: altmax
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: lai_x                                !! Leaf Area Index as calculated in STOMATE 
                                                                                    !! [m2/m2] 
!! xchen
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: lai_cl1                              !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: lai_cl2                              !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: lai_cl3                              !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: lai_cl4                              !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: laitop                               !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: laidow                               !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: VPD                                  !! Vapor Pressure Deficit (kPa) 
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: ave_VPD                              !! Vapor Pressure Deficit (kPa) 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: vcmax_cl1                            !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: vcmax_cl2                            !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: vcmax_cl3                            !! Leaf Area Index [m2/m2] 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: vcmax_cl4                            !! Leaf Area Index [m2/m2] 
  ...
  ! CROPS
  LOGICAL, DIMENSION(:), ALLOCATABLE        :: f_rot_sech
  INTEGER(i_std), DIMENSION(:,:), ALLOCATABLE :: rot_cmd
  REAL(r_std),DIMENSION (:), ALLOCATABLE    :: swdown                               !!downward shortwave radiation
  REAL(r_std),DIMENSION (:), ALLOCATABLE    :: t2mdiag                              !!2m air temperature (K) of daily maximum
  REAL(r_std),DIMENSION (:), ALLOCATABLE    :: evapot_corr                          !!potential evaportranspiration (mm)
  ...
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE    :: fraclut        !! Fraction   of each landuse tile (0-1, unitless)
  REAL(r_std),DIMENSION (:,:),ALLOCATABLE   :: nwdFraclut     !! Fraction of non-woody vegetation in each landuse tile (0-1, unitless)
...
!!  yidi
  ...
  REAL(r_std),DIMENSION(nphs2)  :: phylev              !! phytomer axis
!!  yidi
...
!! yidi
  CALL getin_p('NPH',nphs)
  WRITE(numout,*)'the number of phytomers used by the model is : ', nphs
...
!end gmjc
  ...
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (co2_flux(kjpindex,nvm),stat=ier)
  CALL ipslerr_p(3,'teststomate','Memory allocation Error for co2_flux','Error code',ier)
  ...
  IF (ier.NE.0) CALL ipslerr_p (3,'teststomate', 'rot_cmd cannot be allocated','There is not enough memory', '')
  rot_cmd(:,:) = 0

  ALLOCATE(f_rot_sech(kjpindex),stat=ier)
  IF (ier.NE.0) CALL ipslerr_p (3,'teststomate', 'f_rot_sech cannot be allocated','There is not enough memory', '')
  f_rot_sech(:) = .FALSE.

  ALLOCATE(swdown(kjpindex),stat=ier)
  IF (ier.NE.0) CALL ipslerr_p (3,'teststomate', 'swdown cannot be allocated','There is not enough memory', '')
  swdown(:) = 0

  ALLOCATE(t2mdiag(kjpindex),stat=ier)
  IF (ier.NE.0) CALL ipslerr_p (3,'teststomate', 't2mdiag cannot be allocated','There is not enough memory', '')
  t2mdiag(:) = 0

  ALLOCATE(evapot_corr(kjpindex),stat=ier)
  IF (ier.NE.0) CALL ipslerr_p (3,'teststomate', 'evapot_corr cannot be allocated','There is not enough memory', '')
  evapot_corr(:) = 0
  ...
  totsize_1step = SIZE(soiltile(:,3))*KIND(soiltile(:,3)) + &
       ...
       SIZE(t2mdiag)*KIND(t2mdiag) + &
       SIZE(swdown)*KIND(swdown) + &
       SIZE(evapot_corr)*KIND(evapot_corr) 
...
!-
  ...
  CALL slowproc_initialize (itau,           kjpij,         kjpindex,            date0,          &
                            ...
                            t2m,                                                                &
                            deadleaf_cover, assim_param_x, lai_x, & 
!!xchen
                            ...
                            rest_id_sto,    hist_id_stom,  tot_bare_soil,       fraclut,        &
                            nwdFraclut,                                                         &
...
!---- start again at first forcing state
     ...
     t2mdiag(:) = t2mdiag_fm(:,iisf)
     swdown(:) = swdown_fm(:,iisf)
     evapot_corr(:) = evapot_corr_fm(:,iisf)
...
!-- scale GPP to new LAI
 ...
 &     fraclut, nwdFraclut, t2m, temp_sol, soiltemp, &
...
!-
  IF (printlev_loc>=3) WRITE(numout,*) "Call slowproc for restart."
       CALL slowproc_finalize (itau_fin,   kjpindex,    rest_id_sec, indices,    &
                               ...
                               assim_param_x, frac_age_x,altmax)
!!$  CALL slowproc_main &
